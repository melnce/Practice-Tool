...
async function shot(page: import("@playwright/test").Page, name: string) {
  await page.screenshot({ path: path.join(QA_SHOTS, `${name}.png`), fullPage: true });
}
...
test.describe("S2 — targeting", () => {
  test("single-select spell, multi-select Ralmia, leader, cancel", async ({ page }) => {
    const tracker = trackConsole(page);
    const po = new SvwbPage(page);
    await po.gotoTestMode();
    await po.loadDb();
...
test.describe("S6 — edges", () => {
  test("9-hand burn, full board, ward, aura", async ({ page }) => {
    ...
    await qaStep(page, tracker, async () => {
      ...
      for (let i = 0; i < 9; i++) {
        await page.evaluate(() => window.__svwbTest!.addToHand("first", "10001110"));
      }
    ...
    await qaStep(page, tracker, async () => {
      for (let i = 0; i < 5; i++) {
        await page.evaluate(() => window.__svwbTest!.summonToBoard("first", "10001110"));
      }
