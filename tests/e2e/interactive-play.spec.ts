/**
 * Broader interactive play E2E — paths the audit suite skips.
...
test.describe("Interactive play paths", () => {
  ...
  test("targeted spell via right-click play + board click", async ({ page }) => {
    ...
    await page.locator('#redBoard .card[data-selectable="true"]').first().click();
    await page.waitForTimeout(200);
  ...
  test("fanfare select (nested_effects): play pauses until board click resolves", async ({
    ...
    const selectable = await page.locator(
      '#blueBoard .card[data-selectable="true"]',
    ).count();
    expect(selectable).toBe(1);

    await page.locator('#blueBoard .card[data-selectable="true"]').first().click();
    await page.waitForTimeout(200);
  ...
  test("Ward blocks leader click but allows Ward target selection for spell", async ({
    ...
    const selectableCount = await page.locator(
      '#redBoard .card[data-selectable="true"]',
    ).count();
    expect(selectableCount).toBe(2);

    await page.locator('#redBoard .card[data-selectable="true"]').nth(1).click();
    await page.waitForTimeout(200);
  ...
  test("super-evolve: drag super button then select Golem target", async ({
    ...
    await expect(
      page.locator('#blueBoard .card[data-selectable="true"]'),
    ).toBeVisible({
      timeout: 5000,
    });
    await page.locator('#blueBoard .card[data-selectable="true"]').first().click();
  ...
  test("left-click fusable hand card enters fuse selection", async ({ page }) => {
    ...
    const selectablePartners = await page.locator(
      '#blueHand .card[data-selectable="true"]',
    ).count();
