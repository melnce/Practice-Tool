...
describe('Troue, Heroic Visionary (10461110)', () => {
    ...
    it('should gain Drain when an amulet is engaged', () => {
        // 1. Setup Troue
        const troueDef: any = masterCardDefinitions.find((c: any) => c.id === "10461110");
        ...
        const amuletDef: any = {
            ...
            hasEngage: true,
            engageCost: 0,
            engageEffects: [{ op: "add_to_hand", name: "Fairy", count: 0 }] // specific effect irrelevant, just need to run
        };
        ...
        expect(troue.keywords?.some((k: any) => k === "Drain" || k.name === "Drain")).toBeFalsy();
        ...
        const hasDrainKeyword = troue.keywords?.some((k: any) => k === "drain" || k.name === "drain");
