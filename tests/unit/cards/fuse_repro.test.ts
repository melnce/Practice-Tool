...
describe('Fuse Selection (Bug Repro)', () => {
    ...
    it('Gear of Ambition should have fuse_recipes', () => {
        ...
        log(`Gear of Ambition: ${JSON.stringify({
            name: gear.name,
            type: gear.type,
    ...
    it('should set pendingTargetEffect when starting fuse', async () => {
        ...
        log(`pendingTargetEffect after startFuse: ${JSON.stringify(state.pendingTargetEffect, null, 2)}`);
    ...
    it('should mark pool cards as __uiSelectable', async () => {
        ...
        log(`Cards __uiSelectable: ${state.blueHand.map(c => `${c.name}: ${!!c.__uiSelectable}`).join(', ')}`);
    ...
    it('should allow target selection via resolvePendingTarget', async () => {
        ...
        const { startFuseFromHand, resolvePendingTarget } = await import('../../../src/logic/index');
        startFuseFromHand("blue", gear1.uid);

        log(`Pool before select: ${JSON.stringify(state.pendingTargetEffect?.pool?.map((c: any) => c.name))}`);
        log(`Targeting gear2 UID: ${gear2.uid}`);

        // Simulate clicking on gear2
        resolvePendingTarget(gear2.uid);

        log(`Targets after select: ${JSON.stringify(state.pendingTargetEffect?.targets?.map((c: any) => c.name || c.uid))}`);
