...
import { handleBuffSelf } from '../../../src/logic/effects/self.ts';
...
describe('Lamretta, Sisterly Shepherd (10461120)', () => {
    beforeEach(() => {
        ...
        if (!state.rng) state.rng = { makeUid: () => Math.random().toString(36).substr(2, 9) } as any;
    });

    it('should gain temporary "Can\'t Attack" on evolve via buff_self', () => {
        // 1. Setup Lamretta
        const lamrettaDef: any = masterCardDefinitions.find((c: any) => c.id === "10461120");
        ...
        // 3. Execute
        handleBuffSelf(lamretta, eff as any);
        console.log("Lamretta Keywords:", JSON.stringify(lamretta.keywords, null, 2));
        console.log("Lamretta KeywordState:", JSON.stringify(lamretta.keywordState, null, 2));
