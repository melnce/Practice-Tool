...
function analyzeEffects(effects: Effect[]): {
  expectations: Expectation[];
  conditions: Condition[];
} {
  const expectations: Expectation[] = [];
...
async function main() {
  ...
  for (const spec of specs) {
    for (const scenario of spec.scenarios) {
      triggerCounts[scenario.trigger] =
        (triggerCounts[scenario.trigger] || 0) + 1;
    }
  }
  console.log("\nScenarios by trigger:");
  for (const [trigger, count] of Object.entries(triggerCounts).sort(
    (a, b) => b[1] - a[1],
  )) {
