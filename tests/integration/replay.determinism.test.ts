...
describe("Seeded Fuzzing: Random Action Sequences", () => {
    ...
    it("random mutations with same seed produce identical final hash", () => {
        ...
        for (let i = 0; i < ITERATIONS; i++) {
            ...
            switch (action) {
                case 0:
                    s1.players.first.hp = Math.max(0, s1.players.first.hp - s1.rng.nextInt(3));
                    break;
                case 1:
                    s1.players.second.hp = Math.max(0, s1.players.second.hp - s1.rng.nextInt(3));
                    break;
                case 2:
        ...
        for (let i = 0; i < ITERATIONS; i++) {
            ...
            switch (action) {
                case 0:
                    s2.players.first.hp = Math.max(0, s2.players.first.hp - s2.rng.nextInt(3));
                    break;
                case 1:
                    s2.players.second.hp = Math.max(0, s2.players.second.hp - s2.rng.nextInt(3));
                    break;
                case 2:
