...
describe("Mechanic Contract: stat", () => {
  ...
  describe("give action", () => {
    it("increases attack by specified amount", () => {
      givenGameState({ seed: 1 })
        .withFirstBoard([
          { name: "Target", type: "Follower", attack: 2, defense: 3 },
        ])
        .build();
    ...
    it("gives both attack and defense", () => {
      givenGameState({ seed: 1 })
        .withFirstBoard([
          { name: "Target", type: "Follower", attack: 1, defense: 1 },
        ])
        .build();
    ...
    it("does NOT buff enemy followers when target is ally:follower", () => {
      givenGameState({ seed: 1 })
        .withFirstBoard([
          { name: "Ally", type: "Follower", attack: 1, defense: 1 },
        ])
        .withSecondBoard([
          { name: "Enemy", type: "Follower", attack: 1, defense: 1 },
        ])
        .build();
  ...
  describe("set action", () => {
    it("overrides attack to specified value", () => {
      givenGameState({ seed: 1 })
        .withFirstBoard([
          { name: "Target", type: "Follower", attack: 5, defense: 3 },
        ])
        .build();
    ...
    it("overrides defense to specified value", () => {
      givenGameState({ seed: 1 })
        .withFirstBoard([
          { name: "Target", type: "Follower", attack: 2, defense: 5 },
        ])
        .build();
  ...
  describe("self targeting", () => {
    it("buffs source card when target is self", () => {
      givenGameState({ seed: 1 })
        .withFirstBoard([
          { name: "Source", type: "Follower", attack: 2, defense: 2 },
        ])
        .build();
    ...
    it("self targeting with no source card does nothing", () => {
      givenGameState({ seed: 1 })
        .withFirstBoard([
          { name: "Bystander", type: "Follower", attack: 1, defense: 1 },
        ])
        .build();
