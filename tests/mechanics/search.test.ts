...
describe("Mechanic Contract: search op", () => {
  ...
  describe("basic search", () => {
    it("finds matching card in deck and adds to hand", () => {
      ...
      const foundSpell = thenHand("first").find(
        (c) => c.name === "TargetSpell",
      );
    ...
    it("search with no matches does nothing - deck and hand unchanged", () => {
      givenGameState({ seed: 1 })
        ...
        .withFirstHand([{ name: "InHand", type: "Spell", cost: 1 }])
        .build();
    ...
    it("search with count 0 does nothing", () => {
      givenGameState({ seed: 1 })
        .withFirstDeck([{ name: "A", type: "Spell", cost: 1 }])
        .build();
  ...
  describe("search count", () => {
    ...
    it("takes multiple topmost when count > 1", () => {
      ...
      expect(thenDeck("first").some((c) => c.name === "BottomSpell")).toBe(
        true,
      );
    });
  });
  ...
  describe("hand overflow (excess to graveyard)", () => {
    it("puts excess searched cards in graveyard when hand fills", () => {
      ...
      expect(thenHand("first").filter((c) => c.type === "Spell").length).toBe(
        1,
      );
    ...
    it("all searched cards go to graveyard when hand is already full (9)", () => {
      givenGameState({ seed: 1 })
        .withFirstDeck([{ name: "DeckSpell", type: "Spell", cost: 1 }])
        .withFirstHand([
          { name: "H1", type: "Follower", attack: 1, defense: 1 },
          { name: "H2", type: "Follower", attack: 1, defense: 1 },
          { name: "H3", type: "Follower", attack: 1, defense: 1 },
  ...
  describe("state tracking", () => {
    ...
    it("updates lastSearchedCards array", () => {
      ...
      expect(
        (state as any).lastSearchedCards.every((c: any) => c.type === "Spell"),
      ).toBe(true);
    });
  });
  ...
  describe("zone transitions", () => {
    it("sets card zone to 'hand' when searched", () => {
      givenGameState({ seed: 1 })
        .withFirstDeck([{ name: "Target", type: "Spell", cost: 1 }])
        .build();
  ...
  describe("player targeting", () => {
    it("can search opponent's deck when player: opponent", () => {
      givenGameState({ seed: 1 })
        .withFirstDeck([{ name: "FirstDeck", type: "Spell", cost: 1 }])
        .withSecondDeck([{ name: "SecondDeck", type: "Amulet", cost: 2 }])
        .build();
      ...
      expect(thenHand("second").some((c) => c.name === "SecondDeck")).toBe(
        true,
      );
      // ASSERTION: First player's hand unchanged
      expect(thenHand("first").some((c) => c.name === "SecondDeck")).toBe(
        false,
      );
    });
  });
  ...
  describe("edge cases", () => {
    ...
    it("handles deck with single matching card", () => {
      givenGameState({ seed: 1 })
        .withFirstDeck([{ name: "Only", type: "Spell", cost: 1 }])
        .build();
    ...
    it("handles large deck efficiently", () => {
      ...
      const spellCountBefore = state.players.first.deck.filter(
        (c) => c.type === "Spell",
      ).length;
      ...
      expect(thenDeck("first").filter((c) => c.type === "Spell").length).toBe(
        spellCountBefore - 3,
      );
    });
  });
