...
describe("Mechanic Contract: damage", () => {
  ...
  describe("basic follower damage", () => {
    it("reduces defense by exact amount (direct)", () => {
      givenGameState({ seed: 1 })
        .withSecondBoard([
          { name: "Target", type: "Follower", defense: 5, attack: 1 },
        ])
        .build();
    ...
    it("destroys follower when defense reaches 0", () => {
      givenGameState({ seed: 1 })
        .withSecondBoard([
          { name: "Target", type: "Follower", defense: 3, attack: 1 },
        ])
        .build();
    ...
    it("destroys follower when damage exceeds defense", () => {
      givenGameState({ seed: 1 })
        .withSecondBoard([
          { name: "Target", type: "Follower", defense: 2, attack: 1 },
        ])
        .build();
    ...
    it("damages ONLY enemy follower, not ally", () => {
      ...
      givenGameState({ seed: 1 })
        .withFirstBoard([
          { name: "Ally", type: "Follower", defense: 5, attack: 1 },
        ])
        .withSecondBoard([
          { name: "Enemy", type: "Follower", defense: 5, attack: 1 },
        ])
        .build();
  ...
  describe("leader damage", () => {
    ...
    it("allows HP to go negative (for game-over detection)", () => {
      givenGameState({ seed: 1 }).withSecondHP(3).build();
    ...
    it("damages ENEMY leader, not ally leader", () => {
      ...
      givenGameState({ seed: 1 }).withFirstHP(20).withSecondHP(20).build();
  ...
  describe("random_hits distribution", () => {
    ...
    it("deals multiple random hits totaling correct damage", () => {
      ...
      const totalDefense = thenBoard("second").reduce(
        (sum, c) => sum + (c.defense as number),
        0,
      );
    ...
    it("random hits can kill followers during distribution", () => {
      ...
      const totalRemaining = remainingBoard.reduce(
        (sum, c) => sum + (c.defense as number),
        0,
      );
