...
describe("Regression: Skullfane of Demise", () => {
    ...
    describe("store_count_as -> amount_source variable propagation", () => {
        it("damage effect reads destroy_count from previous destroy effect", () => {
            ...
            const sourceCard = { name: "Skullfane", type: "Follower", uid: "test-skullfane" } as any;
            ...
            const amuletCount = state.players.first.board.filter(c => c?.type === "Amulet").length;
            ...
            const leaderHPAfter = (state.players.second as any).leaderHP ?? (state.players.second as any).hp ?? 20;
        ...
        it("deals 0 damage when no amulets destroyed", () => {
            ...
            const sourceCard = { name: "Skullfane", type: "Follower", uid: "test-skullfane2" } as any;
        ...
        it("context.variables is populated after destroy effect", () => {
            ...
            const sourceCard = { name: "Test", type: "Follower", uid: "test-card" } as any;
            ...
            const sharedContext: { variables: Record<string, number> } = { variables: {} };
            runEffects(effects, "first", sourceCard, sharedContext);
