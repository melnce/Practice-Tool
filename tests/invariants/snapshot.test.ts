...
describe("Snapshot Omission", () => {
    ...
    it("should fail if new underscore keys appear that arent in INTERNAL_CACHE_KEYS", () => {
        ...
        const unexpectedUnderscoreKeys = stateKeys.filter(
            k => k.startsWith("_") && !INTERNAL_CACHE_KEYS.has(k) && !ALLOWED_UNDERSCORE_KEYS.has(k)
        );
...
describe("Undo/Redo Determinism", () => {
    ...
    it("should maintain determinism across multiple undo/redo cycles", () => {
        ...
        for (let i = 0; i < 5; i++) {
            traces.push(`uid:${state.rng.makeUid()},rand:${state.rng.nextFloat().toFixed(8)}`);
        }
        ...
        for (let i = 0; i < 5; i++) {
            postCheckpointTraces.push(`uid:${state.rng.makeUid()},rand:${state.rng.nextFloat().toFixed(8)}`);
        }
        ...
        for (let cycle = 0; cycle < 3; cycle++) {
            ...
            for (let i = 0; i < 5; i++) {
                replayedTraces.push(`uid:${state.rng.makeUid()},rand:${state.rng.nextFloat().toFixed(8)}`);
            }
