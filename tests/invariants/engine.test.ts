...
function stepSignature(
  ...
  return [
    ...
    stats.firstHP,
    stats.secondHP,
    stats.firstBoardSize,
    stats.secondBoardSize,
    stats.firstHandSize,
    stats.secondHandSize,
    actionsLen,
    actionSeq,
    zoneVersion,
  ].join("|");
...
describe("Engine Invariants", () => {
  describe("Determinism", () => {
    it("should produce identical trace hash for same seed (FNV-1a)", () => {
      ...
      const run = () => {
        ...
        for (let i = 0; i < STEPS; i++) {
          ...
          h = fnv1a32(
            h.toString(16) + ":" + stepSignature(stats, actions.length),
          );
        }
        return h;
      };
    ...
    it("different seeds usually produce divergent traces", () => {
      ...
      if (!diverged) {
        console.warn(
          "Divergence test inconclusive: traces identical (synthetic state converges)",
        );
      }
