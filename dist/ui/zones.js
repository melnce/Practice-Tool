...
function getSpellboostCount(card) {
    ...
    const hasSpellboost = Array.isArray(card.keywords) &&
        card.keywords.some(k => (typeof k === "string" && k.toLowerCase() === "spellboost") ||
...
export function renderZone(containerId, cards, state, rerender, clickable = false, onClick) {
    ...
    cards.forEach((card, i) => {
        ...
        const { div, imageWrapper, topLeft, spellboostContainer, bottomLeft, bottomRight } = buildCardDiv(card, `${containerId}-${i}`);
        ...
        // === overlays
        if (isBoard) {
            if (card.hasEvolved && card.evoType === "super")
                div.classList.add("super-evo");
        ...
        if (isBoard && isFollower && card.can_attack && !card.hasAttacked && isMyBoard) {
            if (!card.cantAttack && !card.cantAttackFollowers && !card.cantAttackLeaders && !card.hasCantAttack) {
                if (card.isRush && card.justPlayed)
                    div.classList.add("rush-glow");
                else
        ...
        if (isHand) {
            if (state.phase === "mulligan" && card.__mulliganSelectable) {
                ...
                div.addEventListener('click', (e) => {
                    if (card.__uiSelectable)
                        return; // let selection handler take it
                    const isPlayersTurn = (isBlueHand && state.isBlueTurn) || (!isBlueHand && !state.isBlueTurn);
                    if (!isPlayersTurn)
                        return;
                    // Check for Fuse
                    ...
                    if (hasFuseRecipes || hasFortifierFuse) {
                        ...
                        logic().then(({ startFuseFromHand, runEffects }) => {
                            if (hasFuseRecipes)
                                startFuseFromHand(ownerSide, card.uid);
                            else
                                runEffects([{ op: "start_fortifier_fuse", initiator_uid: card.uid }], ownerSide, card);
                        });
                        e.stopPropagation();
                    }
                });
        ...
        if (isBoard && isFollower) {
            const player = isBlueBoard ? "blue" : "red";
            const isMyTurn = (state.isBlueTurn && isBlueBoard) || (!state.isBlueTurn && isRedBoard);
            const isEnemyBoard = !isMyTurn;
            if (isMyTurn && card.can_attack)
                enableAttackerDrag(div, player, i);
            if (isEnemyBoard)
                enableEnemyFollowerDrop(div, null, i, state, isRedBoard);
        }
        // Engage (amulets)
