...
export function formatCardTooltip(card, owner = null) {
    ...
    if (Array.isArray(card?._fusedLootNames) && card._fusedLootNames.length > 0) {
        const uniq = Array.from(new Set(card._fusedLootNames
            .map(n => String(n || "").trim())
            .filter(Boolean)));
    ...
    let rallySection = "";
    const wantsRally = String(card?.name).toLowerCase() === "gildaria, anathema of peace" ||
        /\brally\b/i.test(desc);
    if (wantsRally) {
        const side = owner ?? null;
        const m = desc.match(/Rally\s*\((\d+)\)/i);
        const need = m ? parseInt(m[1], 10) : null;
        const initial = side === "blue" ? (state.blueRally | 0) :
    ...
    div.onmouseenter = () => {
        ...
        if (rallyLine && rallyValue) {
            ...
            function tick() {
                ...
                const curr = side === "blue" ? (state.blueRally | 0) :
                    side === "red" ? (state.redRally | 0) : 0;
                if (curr !== last) {
