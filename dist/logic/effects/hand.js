...
export function handleDiscardAllExceptNamed(eff, owner) {
    ...
    for (let i = hand.length - 1; i >= 0; i--) {
        const c = hand[i];
        if (c && keepSet.has(String(c.name)))
            continue; // keep
    ...
    if (discarded > 0) {
        ...
        if (owner === "blue")
            state.blueShadows += discarded;
        else
            state.redShadows += discarded;
    }
}
/**
 ...
 */
export function handleDiscardSelectHand(eff, owner, resumeEffects = []) {
    const n = Math.max(0, parseInt(eff.count ?? 1, 10));
    const hand = owner === "blue" ? state.blueHand : state.redHand;
 ...
 */
export function handleTransformInHand(eff, owner) {
    const hand = owner === "blue" ? state.blueHand : state.redHand;
    ...
    for (let i = hand.length - 1; i >= 0; i--) {
        ...
        let matches = true;
        // Check class filter
        if (filter.class && card.class !== filter.class) {
            matches = false;
        }
