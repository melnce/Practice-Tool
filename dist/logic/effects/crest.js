...
function findCrest(owner, name) {
    ...
    return list.find((c) => String(c.name).toLowerCase() === String(name).toLowerCase());
}
/** Normalize to array */
function toArray(x) {
...
export function handleGainCrest(eff, owner) {
    ...
    const targetOwner = eff.player === "opponent" ? (owner === "blue" ? "red" : "blue") : owner;
...
export function tickCrests(owner) {
    ...
    if (!Array.isArray(crests) || !crests.length)
        return [];
    ...
    for (let i = 0; i < crests.length; i++) {
        const c = crests[i];
        if (!Number.isFinite(c?.countdown))
            continue;
        c.countdown -= 1;
        if (c.countdown <= 0) {
...
export function processCrestEvent(owner, event) {
    ...
    for (const crest of crests) {
        ...
        for (const t of crest.triggers) {
            if (t.event !== event)
                continue;
            if (t.once_per_turn && t.usedThisTurn)
                continue;
            if (t.effects?.length)
                out.push(...t.effects);
            if (t.once_per_turn)
...
export function completeCrest(crest, owner, context = {}) {
    ...
    if (!Array.isArray(list) || !list.includes(crest))
        return; // already gone
