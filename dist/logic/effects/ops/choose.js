...
export function handleChoose(eff, owner, sourceCard, effectsQueue) {
    console.group("[CHOICE DEBUG] Handling choose effect (multi-pick)");
    const options = Array.isArray(eff?.options) ? eff.options : [];
    const baseSelect = Math.max(1, parseInt(eff?.select_count ?? 1));
    ...
    const unique = eff?.unique !== false; // default true
    ...
    const scoreOption = (opt) => {
        ...
        for (const e of ops) {
            ...
            if (op === "damage" || op === "damage_all" || op === "damage_random" ||
                op === "destroy" || op === "destroy_all" || op === "destroy_random")
                s += 6;
            if (op === "summon_named" || op === "reanimate")
                s += 5;
            if (op === "buff" || op === "buff_self")
                s += 3;
            if (op === "draw" || op === "draw_filtered" || op === "add_to_hand")
                s += 3;
            if (op === "recover_pp")
                s += 2;
            if (op === "leader_barrier")
                s += 2;
            // small bias for effects mentioning enemy in target
            if (typeof e?.target === "string" && /enemy/i.test(e.target))
                s += 1;
        }
    ...
    if (isAIMode()) {
        ...
        while (picked.length < selectCount && pool.length) {
            ...
            candidates.sort((a, b) => scoreOption(b) - scoreOption(a));
        ...
        for (let i = 0; i < picked.length; i++) {
            try {
                fireTrigger("select_mode", owner, { source: sourceCard || null });
            }
            catch { }
        }
        const combined = [];
    ...
    const pickOnce = (pool) => {
        ...
        showChoiceModal(roundPool, (selectedIndex) => {
    ...
    const finalize = () => {
        ...
        doAction("Confirm Choice", () => {
            ...
            console.groupEnd();
        }, { owner, picks: picked.map(p => p?.label || p?.name || "(opt)") }, { owner, picks: picked.map(opt => opt?.label || opt?.name || "(opt)") }, { autoRender: false });
    };
    // Start first round
    pickOnce(available);
}
