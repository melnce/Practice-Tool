...
function isAlly(card, owner) {
    const board = owner === "blue" ? state.blueBoard : state.redBoard;
    return board?.includes(card);
}
function isOwnTurn(owner) {
...
export function handleDamageAll(eff, owner, sourceCard = null) {
    const pool = getPool(eff.target, owner, sourceCard, eff.condition);
    ...
    if (String(eff.target || "").includes("leader")) {
        const targetPlayer = eff.target.includes("enemy")
            ? (owner === "blue" ? "red" : "blue")
            : owner;
...
export function handleDamage(eff, owner, sourceCard, effectsQueue, context = {}) {
    ...
    const selectCount = parseInt(eff.select) || 0;
    if (selectCount > 0) {
        // --- MODIFIED LOGIC ---
...
export function handleDamageRandom(eff, owner) {
    const amt = resolveAmountWithOverflow(eff, owner, {});
    let hits = Math.max(1, parseInt(eff.count || 1));
    while (hits-- > 0) {
        // Rebuild pool each hit (accounts for deaths mid-sequence)
        const pool = getPool(eff.target, owner).filter(c => c && c.type === "Follower");
        if (!pool.length)
            break;
 ...
 */
export function handleDamageSplitSequential(eff, owner) {
    ...
    const pool = getPool(eff.target, owner).filter(c => c.type === "Follower");
    if (!pool.length)
        return;
    // Apply damage sequentially
    for (const target of pool) {
        if (damageToDeal <= 0)
            break; // All damage has been dealt
        const damageForThisTarget = Math.min(damageToDeal, target.defense);
...
function resolveAmountWithOverflow(eff, owner, context = {}) {
    const resolveToken = (val) => {
        ...
        if (s === "{attacker.attack}")
            return parseInt(context.attacker?.attack || 0, 10) || 0;
        if (s === "{defender.defense}")
            return parseInt(context.defender?.defense || 0, 10) || 0;
    ...
    const ofRaw = eff.amount_overflow ?? eff.overflow_amount ?? baseRaw;
    const baseAmt = resolveToken(baseRaw);
    const ofAmt = resolveToken(ofRaw);
    return isOverflow(owner) ? ofAmt : baseAmt;
}
export function handleDamageFollowerOrLeader(eff, owner, sourceCard, effectsQueue) {
    const amt = resolveAmountWithOverflow(eff, owner);
    state.pendingTargetEffect = {
        eff: {
            ...eff,
        ...
        targets: [],
        selectCount: 1,
        pool: getPool(eff.target, owner, sourceCard, eff.condition, { isTargetedEffect: true }),
        resumeEffects: effectsQueue,
        canTargetLeader: eff.can_target_leader
    };
    logEvent("damageFoL_select", { owner, canTargetLeader: !!eff.can_target_leader });
    const pool = getPool(eff.target, owner);
    if (pool.length) {
        highlightSelectable(pool);
    }
    return "pending";
...
export function handleDamageAllByAlliedGolems(eff, owner) {
    // Count allied Golem followers on field
    const board = owner === "blue" ? state.blueBoard : state.redBoard;
    const golemCount = board.filter(c => c.type === "Follower" &&
        ((Array.isArray(c.tribes) && c.tribes.includes("Golem")) ||
            /golem/i.test(c.name))).length;
    if (golemCount <= 0)
        return;
    ...
    const pool = (enemy === "blue" ? state.blueBoard : state.redBoard)
        .filter(c => c.type === "Follower");
    for (const t of pool)
        dealDamage(t, golemCount);
    cleanupDead();
}
export function handleDamageSplitFixed(eff, owner) {
...
export function handleDamageRandomSelectedDefense(eff, owner, _sourceCard, _effectsQueue, context = {}) {
...
export function handleDamageSplitAllEnemies(eff, owner, sourceCard = null) {
    ...
    for (const target of snapshotFollowers) {
        ...
        const curDef = parseInt(target.defense, 10) || 0;
        if (curDef <= 0)
            continue;
...
export function handleDamageHighestDefense(eff, owner, sourceCard = null) {
    const amt = parseInt(eff.amount) || 0;
    // Followers first
    if (eff.target === "follower") {
        const allFollowers = [...(state.blueBoard || []), ...(state.redBoard || [])]
            .filter(c => c && c.type === "Follower");
        if (!allFollowers.length)
            return;
        const maxDef = Math.max(...allFollowers.map(c => parseInt(c.defense) || 0));
        const targets = allFollowers.filter(c => (parseInt(c.defense) || 0) === maxDef);
