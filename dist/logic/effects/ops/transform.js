...
function locateZone(target) {
    if (!target)
        return null;
    if (state.blueBoard?.includes(target))
        return "blueBoard";
    if (state.redBoard?.includes(target))
        return "redBoard";
    if (state.blueHand?.includes(target))
        return "blueHand";
    if (state.redHand?.includes(target))
        return "redHand";
    return null;
}
/**
 ...
 */
export function transformTarget(target, intoName) {
    if (!target || !intoName) {
    ...
    if (c.type === "Follower") {
        ...
        if (c.base_defense == null)
            c.base_defense = c.defense;
        if (c.peak_defense == null)
        ...
        applyKeywordsFromList(c);
        if (c.hasCountdown)
            c.countdown = Number(c.countdown || 0);
    }
 ...
 */
export function transformHandTarget(target, intoName) {
    if (!target || !intoName) {
    ...
    if (target.cost_mod !== undefined)
        c.cost_mod = target.cost_mod;
...
export function transformAnywhere(target, intoName) {
    const zone = locateZone(target);
    if (zone === "blueBoard" || zone === "redBoard")
...
export function transformRandomSpellInHand(owner, intoName = "Ersatz Elimination") {
