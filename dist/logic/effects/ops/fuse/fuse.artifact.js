...
// ---------- starters ----------
export function startGearMultiSelect(owner, initiator) {
...
export function startFortifierFuse(owner, initiator) {
    const hand = handOf(owner);
...
export function startAlphaSelect(owner, initiator) {
    const hand = handOf(owner);
    const pool = hand.filter(c => c.uid !== initiator.uid &&
...
export function fuse_finalize_gear_multi(owner, initiatorUid, partners, resultName) {
    const hand = handOf(owner);
...
export function fuse_finalize_fortifier(owner, initiatorUid, partners) {
    const hand = handOf(owner);
    const iIdx = hand.findIndex(c => c?.uid === initiatorUid);
    if (iIdx === -1) {
        clearSelectableFlags();
...
export function fuse_finalize_alpha(owner, initiatorUid, partners) {
    const hand = handOf(owner);
    ...
    const idxOf = (uid) => hand.findIndex(c => c?.uid === uid);
    if (hasBeta && hasGamma) {
