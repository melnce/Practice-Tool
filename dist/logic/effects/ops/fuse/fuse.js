...
function filterByPartnerFilters(candidates, filters = []) {
    ...
    const pass = (card, f) => {
        ...
        if (wantType && cType !== wantType)
            return false;
        if (wantClass && cClass !== wantClass)
            return false;
...
function buildPartnerPool(owner, initiator) {
    ...
    for (let rIndex = 0; rIndex < recipes.length; rIndex++) {
        ...
        const pool = filterByPartnerFilters(hand.filter(c => c?.uid !== initiator.uid), r.partner_filters);
        if (pool.length)
            return { recipe: r, recipeIndex: rIndex, pool };
    }
    return null;
}
// -------------------- central starter --------------------
export function opStartFuseFromCard(eff, owner) {
    const hand = handOf(owner);
...
export function fuse_finalize_generic(owner, initiatorUid, partnerCard, resultSpec) {
    const hand = handOf(owner);
