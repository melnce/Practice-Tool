...
function getSpellboostKeyword(card) {
...
function transformSelfInHand(owner, c, targetName) {
 ...
 */
function handleSpellboostKeywordEffects(owner, c) {
    const kws = Array.isArray(c.keywords) ? c.keywords : [];
    for (const kw of kws) {
        if (kw?.name !== "Spellboost" || !Array.isArray(kw.effects))
            continue;
 ...
 */
export function spellboostHand(owner, times = 1, targetCard = null) {
    ...
    for (let i = 0; i < t; i++) {
        ...
        for (const c of board) {
            const kw = getSpellboostKeyword(c);
            if (!kw || !Array.isArray(kw.effects) || kw.effects.length === 0)
                continue;
            for (const effect of kw.effects) {
                if (effect.op === "stormy_blast_counter") {
