...
function filterArtifactFollowersHand(owner, maxCost) {
    const hand = owner === "blue" ? state.blueHand : state.redHand;
    return hand.filter(c => c?.type === "Follower" &&
...
function seedCountersFromKeywords(card) {
...
function safeClone(value, seen = new WeakSet()) {
    ...
    for (const [k, v] of Object.entries(value)) {
        ...
        if (k === "__state" || k === "__dom" || k === "element")
            continue;
        if (v === state)
            continue; // direct reference to global state
        out[k] = safeClone(v, seen);
    }
    return out;
}
// Return how many earth counters a card *starts* with, based on its keywords
function startingEarthFromKeywords(cardData) {
    let n = 0;
...
function findEarthSigilTarget(board) {
    let brew = null;
    let sediment = null;
...
function tryMergeIntoExistingEarthSigil(board) {
...
function dedupeEarthSigils(board) {
...
function mergeSigils(board, sigilsToMerge) {
    if (sigilsToMerge.length <= 1)
        return;
...
function makeCardFromDB(cardData, owner) {
    const card = JSON.parse(JSON.stringify(cardData));
    card.uid = makeUid();
    card.owner = owner;
    if (isFollower(card))
        initFollower(card);
    else if (isAmulet(card))
        initAmulet(card);
    return card;
}
function pushToBoard(board, owner, card) {
...
// =============== Public API ===============
export function summonNamed(eff, owner) {
    state.lastSummoned = [];
    const name = String(eff?.name || "").trim();
    let count = parseInt(eff?.count);
...
export function reanimateSummon(c, owner) {
    ...
    if (!copy.tribes.includes("Departed"))
        copy.tribes.push("Departed");
...
export function summonRandomFromDeck(eff, owner) {
    // Desired number
    let want = parseInt(eff?.count ?? 1);
...
export function handleSummonDestroyedAmuletHighestBaseCost(owner) {
    ...
    const withBase = destroyedAmulets.map(g => {
        ...
        const baseCost = parseInt(base?.cost, 10) || 0;
        if (baseCost > maxBase)
            maxBase = baseCost;
        return { g, baseCost };
    });
...
export function handleSelectHandSummonArtifactCopiesEOT(eff, owner, effectsQueue) {
    const maxCost = Number(eff.max_cost ?? 5);
    const hand = owner === "blue" ? state.blueHand : state.redHand;
    ...
    state.pendingTargetEffect = {
        eff: { ...eff, op: "select_hand_summon_artifact_copies_eot_destroy" },
        owner,
        sourceCard: null,
...
export function summonExactCopyFromHand(srcCard, owner, position = "right") {
    if (!srcCard)
        return null;
    ...
    if (clone.type === "Follower") {
        ...
        if (clone.base_defense == null)
            clone.base_defense = clone.defense;
        if (clone.peak_defense == null)
...
export function handleSelectHandSummonArtifactCopy(eff, owner, effectsQueue) {
    const maxCost = Number(eff.max_cost ?? 5);
    const hand = owner === "blue" ? state.blueHand : state.redHand;
    const pool = (hand || []).filter(c => {
        if (!c || c.type !== "Follower")
    ...
    if (!pool.length)
        return;
    state.pendingTargetEffect = {
        eff: { ...eff, op: "select_hand_summon_artifact_copy" }, // resolved in resolveTarget.js
        owner,
        sourceCard: null,
        resumeEffects: effectsQueue,
        pool,
...
export function summonExactCopy(sourceCard, owner) {
    ...
    if (!pushToBoard(board, owner, clone))
        return null;
