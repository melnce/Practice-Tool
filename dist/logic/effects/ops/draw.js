 ...
 * -----------------------------
 */
export function handleDraw(eff, owner) {
    const n = clampInt(eff.count, 1);
...
export function handleDrawOpponent(eff, owner) {
...
export function handleDrawNamed(eff, owner) {
    const name = toLowerSafe(eff.name);
    if (!name)
        return;
    const deck = owner === "blue" ? state.blueDeck : state.redDeck;
    const hand = owner === "blue" ? state.blueHand : state.redHand;
...
export function handleDrawAllNamedWithKeyword(eff, owner) {
    const name = toLowerSafe(eff.name);
    const kw = toLowerSafe(eff.keyword || "storm");
    if (!name)
        return;
    const deck = owner === "blue" ? state.blueDeck : state.redDeck;
    const hand = owner === "blue" ? state.blueHand : state.redHand;
 ...
 * -----------------------------
 */
export function handleDrawFiltered(eff, owner) {
    const want = clampInt(eff.count, 1);
    if (!want)
        return;
    const deck = owner === "blue" ? state.blueDeck : state.redDeck;
    const hand = owner === "blue" ? state.blueHand : state.redHand;
    ...
    const matches = (c) => {
        ...
        if (cls && cClass !== cls)
            return false;
        if (costEq != null && cCost !== costEq)
            return false;
        if (costLte != null && !(cCost <= costLte))
            return false;
        if (costGte != null && !(cCost >= costGte))
            return false;
        if (atkEq != null && cAtk !== atkEq)
            return false;
        if (atkLte != null && !(cAtk <= atkLte))
            return false;
        if (atkGte != null && !(cAtk >= atkGte))
            return false;
        if (defEq != null && cDef !== defEq)
            return false;
        if (defLte != null && !(cDef <= defLte))
            return false;
        if (defGte != null && !(cDef >= defGte))
            return false;
        if (tribeList.length) {
            const cardTribes = Array.isArray(c.tribes)
                ? c.tribes.map(toLowerSafe)
                : [];
            const hasOne = tribeList.some((t) => cardTribes.includes(t));
            if (!hasOne)
                return false;
        }
        return true;
    ...
    if (!idxs.length)
        return;
    const mode = toLowerSafe(eff.mode) || "topmost";
    let chosenIdxs;
    if (mode === "random") {
    ...
    for (const ix of chosenIdxs) {
        ...
        trackLastDrawn(picked);
        if (!pushToHand(hand, picked))
            break;
    }
    logEvent("tutorFiltered", { owner, mode, moved: chosenIdxs.length });
}
/** Example: combo-based tutor */
export function handleDrawComboFollower(eff, owner) {
    const combo = owner === "blue"
        ? (state.bluePlaysThisTurn || 0)
        : (state.redPlaysThisTurn || 0);
    const drawEff = {
        count: eff.count ?? 1,
        mode: eff.mode || "topmost",
        filters: {
            type: "follower",
            cost_eq: combo,
        },
    };
    handleDrawFiltered(drawEff, owner);
}
/** -----------------------------
 * 4) Future-proof hooks
 * -----------------------------
 */
export function handlePeekTop(eff, owner) {
    const x = clampInt(eff.count, 1);
    const deck = owner === "blue" ? state.blueDeck : state.redDeck;
    const peek = deck.slice(Math.max(0, deck.length - x));
    state.peekTop = { owner, cards: peek.map((c) => ({ ...c })) };
    logEvent("peekTop", { owner, count: x });
}
export function handlePutOnTopOrBottom(eff, owner) {
    const which = toLowerSafe(eff.which) || "top";
    const card = eff.card;
    if (!card)
        return;
    const deck = owner === "blue" ? state.blueDeck : state.redDeck;
