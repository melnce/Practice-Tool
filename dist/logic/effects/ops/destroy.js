...
export function handleDestroyHighest(eff, owner) {
    const pool = getPool(eff.target, owner).filter((c) => c.type === "Follower" && (parseInt(c.defense, 10) || 0) > 0);
    if (!pool.length)
        return;
    const highest = Math.max(...pool.map((c) => parseInt(c.attack, 10) || 0));
    const top = pool.filter((c) => (parseInt(c.attack, 10) || 0) === highest);
    let n = Math.max(1, parseInt(eff.count || 1, 10));
...
export function handleDestroyAll(eff, owner, sourceCard = null, context = {}) {
    let pool = getPool(eff.target || "all:follower", owner, sourceCard, eff.condition, context);
    pool = pool.filter((c) => c &&
        ((c.type === "Follower" && (parseInt(c.defense, 10) || 0) > 0) ||
            c.type === "Amulet"));
    if (!pool.length)
        return;
    for (const card of pool) {
        ...
        if (card.type === "Follower") {
            if (isSuperProtected(card, cardOwner))
                continue;
...
export function handleDestroy(eff, owner, effectsQueue, context = {}, sourceCard = null) {
    if (String(eff.target).toLowerCase().startsWith("selected")) {
        const targets = getPool(eff.target, owner, null, eff.condition, context);
        let count = 0;
        for (const target of targets) {
            if (resolveDestroy(target, owner))
                count++;
        }
        cleanupDead();
        return count;
...
export function resolveDestroy(target, owner) {
    if (!target)
        return;
    const inferredOwner = state.blueBoard.includes(target)
        ...
        : state.redBoard.includes(target)
            ? "red"
            : owner;
    if (target.cannotBeDestroyed)
        return;
    if (isSuperProtected(target, inferredOwner))
        return;
...
export function destroyAlliedAmulets(owner) {
    const board = owner === "blue" ? state.blueBoard : state.redBoard;
...
export function handleDestroyRandom(eff, owner, context = {}) {
    // 1) base pool
    let pool = getPool(eff.target || "enemy:follower", owner, null, eff.condition, context).filter((c) => c && c.type === "Follower" && (parseInt(c.defense, 10) || 0) > 0);
    if (!pool.length)
        return;
    // 2) excludes
    const excludes = new Set();
    const ex = eff.exclude;
    ...
    while (n-- > 0 && pool.length) {
        ...
        const cardOwner = state.blueBoard.includes(pick)
            ...
            : state.redBoard.includes(pick)
                ? "red"
                : null;
        if (cardOwner && isSuperProtected(pick, cardOwner))
            continue;
