...
export function handleHealLeader(owner, eff) {
    ...
    const targetPlayerIsBlue = (eff.player || "self") === "self" ? owner === "blue" : owner !== "blue";
...
export function handleRecoverPP(owner, eff) {
    ...
    const targetIsBlue = (eff.player || "self") === "self" ? owner === "blue" : owner !== "blue";
...
export function grantLeaderBarrier(owner, _charges = 1) {
    ...
    if (state[key])
        return; // already has Barrier
    const s = state; // Allow dynamic access for now or update GameState to have index signature
    s[key] = 1;
...
export function popLeaderBarrier(owner, reason = "damage_prevent") {
    ...
    s[key] = 0;
    const fxKey = owner === "blue" ? "blueLeaderBarrierPopped" : "redLeaderBarrierPopped";
    s[fxKey] = reason;
    return true;
}
...
export function applyLeaderDamage(owner, amount) {
    ...
    if (popLeaderBarrier(owner, "leader_hit"))
        return 0;
    const s = state;
    const cur = s[hpKey] | 0;
...
export function handleLeaderBarrierOp(owner, eff) {
    const target = (eff.player || "self") === "self"
        ? owner
        : owner === "blue"
            ? "red"
            : "blue";
