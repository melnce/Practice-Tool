...
const _seenLootFuseThisTurn = new WeakMap();
// --- Once-per-turn helpers ---
function _didRunOnceThisTurn(hostCard, turnNumber, key) {
    if (!key)
        return false;
    if (!hostCard.__onceByTurn)
        hostCard.__onceByTurn = Object.create(null);
    return hostCard.__onceByTurn[key] === turnNumber;
}
function _markRanThisTurn(hostCard, turnNumber, key) {
    if (!key)
        return;
    if (!hostCard.__onceByTurn)
        hostCard.__onceByTurn = Object.create(null);
    hostCard.__onceByTurn[key] = turnNumber;
}
export function fireTrigger(eventName, activePlayer, context = {}) {
    const _turnToken = Number.isFinite(state.turnNumber) ? state.turnNumber
    ...
    if (eventName === "loot_fused" && context?.initiator) {
        ...
        _seenLootFuseThisTurn.set(context.initiator, _turnToken);
    }
    const enteringCard = context?.enteringCard ?? null;
    const enteringOwner = enteringCard
        ...
        if (Array.isArray(crests)) {
            for (const crest of crests) {
                ...
                for (const cTrig of crestTriggers) {
                    if (!cTrig || cTrig.event !== eventName)
                        continue;
                    ...
                    if (eventName === "ally_follower_enter") {
                        ...
                        if (enteredOwner !== activePlayer)
                            continue; // must be an ally
                        if (cond.not_self && entered.uid === crest.uid)
                            continue;
                        if (cond.tribe) {
                        ...
                        if (cTrig.once_per_turn) {
                            ...
                            if (_didRunOnceThisTurn(crest, _turnToken, key)) {
                            ...
                            _markRanThisTurn(crest, _turnToken, key);
                        }
                        else {
                            logEvent("trigger", { event: eventName, card: crest?.name });
                            runEffects(list, activePlayer, crest, context);
    ...
    for (const { card, owner, source } of zones) {
        ...
        for (const trigger of card.triggers) {
            ...
            if (eventName === "self_damaged") {
                ...
                if (!damaged)
                    continue;
                if (card.uid !== damaged.uid)
                    continue; // must be THIS follower
                const cond = trigger.condition || {};
                if (cond.still_alive && (parseInt(damaged.defense, 10) || 0) <= 0)
                    continue;
                if (cond.own_turn && owner !== state.activePlayer)
                    continue;
            ...
            if (eventName === 'ally_follower_played') {
                ...
                if (!played || played.type !== 'Follower')
                    continue;
                const cond = trigger.condition || {};
                ...
                if (cond.cost_changed) {
                    ...
                    if (!changed) {
                        const printed = Number.isFinite(played.base_cost)
                            ? Number(played.base_cost)
                            : (parseInt(played.cost, 10) || 0);
                        const current = parseInt(played.cost, 10) || 0;
                        const handMod = parseInt(played.cost_mod, 10) || 0;
                        changed = (handMod !== 0) || (Number.isFinite(played.base_cost) && current !== printed);
                    }
                    if (!changed)
                        continue;
                }
                ...
                if (cond.name) {
                    if (String(played.name) !== String(cond.name))
                        continue;
                }
                logEvent("trigger", { event: eventName, card: card?.name });
                runEffects([...trigger.effects], owner, card, { ...context });
            ...
            const needSource = trigger.source || (eventName === 'ally_follower_enter' ? 'board' : null);
            if (needSource && needSource !== source)
                continue;
            ...
            if (eventName === 'engage' && owner !== activePlayer)
                continue;
            // --- For ally_follower_enter: default to ally-only (robust) ---
            if (eventName === 'ally_follower_enter') {
                if (!enteringCard)
                    continue; // nothing entering -> ignore
                if (owner !== enteringOwner)
                    continue; // must be same side as host
                if (source !== 'board')
                    continue; // host must be on board
            }
            // --- For enemy_follower_enter: host on board, and host owner ≠ entering owner
            if (eventName === "enemy_follower_enter") {
                if (!enteringCard)
                    continue; // nothing entered
                if (source !== "board")
                    continue; // host must be on board
                const hostOwner = owner; // <- define properly
                if (hostOwner === enteringOwner)
                    continue; // same side => not enemy
            ...
            const cond = trigger.condition || {};
            // whose_turn
            if (cond.whose_turn === 'owner' && activePlayer !== owner)
                continue;
            if (cond.whose_turn === 'opponent' && activePlayer === owner)
                continue;
            // is_ally (if provided, otherwise ally_follower_enter already enforced)
            if (typeof cond.is_ally === 'boolean' && enteringCard) {
                if (cond.is_ally && owner !== enteringOwner)
                    continue;
                if (!cond.is_ally && owner === enteringOwner)
                    continue;
            }
            ...
            if (cond.cost_changed && enteringCard) {
                const printed = Number.isFinite(enteringCard.base_cost)
                    ? Number(enteringCard.base_cost)
                    : Number(enteringCard.cost) || 0;
                const current = Number(enteringCard.cost) || 0;
                const handMod = Number(enteringCard.cost_mod) || 0;
                // Treat as "changed" iff some effect altered the hand cost (cost_mod ≠ 0)
                // or the printed/base cost was explicitly overwritten.
                const changed = (handMod !== 0) || (Number.isFinite(enteringCard.base_cost) && current !== printed);
                if (!changed)
                    continue;
            }
            // name
            if (cond.name && enteringCard) {
                if (String(enteringCard.name) !== String(cond.name))
                    continue;
            }
            // --- Host card stat gates (for events like end_of_turn on the host) ---
            if (typeof cond.attack_lte === "number" && (parseInt(card.attack, 10) || 0) > cond.attack_lte)
                continue;
            if (typeof cond.attack_gte === "number" && (parseInt(card.attack, 10) || 0) < cond.attack_gte)
                continue;
            if (typeof cond.defense_lte === "number" && (parseInt(card.defense, 10) || 0) > cond.defense_lte)
                continue;
            if (typeof cond.defense_gte === "number" && (parseInt(card.defense, 10) || 0) < cond.defense_gte)
                continue;
            if (cond.still_alive === true && (parseInt(card.defense, 10) || 0) <= 0)
                continue;
            // not_self
            if (cond.not_self && enteringCard && enteringCard.uid === card.uid)
                continue;
            // your_turn_only
            if (trigger.your_turn_only && owner !== activePlayer)
                continue;
            // once_per_turn (check first, mark after successful fire)
            if (trigger.once_per_turn) {
                const key = trigger.once_key || `${trigger.event}_once`;
                if (_didRunOnceThisTurn(card, _turnToken, key)) {
                    logEvent("triggerSkip", {
                        event: eventName,
                        card: card?.name,
                        reason: "once_per_turn"
                ...
                _markRanThisTurn(card, _turnToken, key);
                continue;
            }
            logEvent("trigger", { event: eventName, card: card?.name });
            runEffects([...trigger.effects], owner, card, { ...context, enteringOwner });
