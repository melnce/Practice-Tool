...
function _isCardDamaged(c) {
    ...
    const full = Number.isFinite(c?.potential_defense) ? c.potential_defense :
...
function getCardSide(c) {
    if (state.blueBoard?.includes(c))
        return "blue";
    if (state.redBoard?.includes(c))
        return "red";
    if (state.blueHand?.includes(c))
        return "blue"; // also check hands for completeness
    if (state.redHand?.includes(c))
        return "red";
    return c?.owner ?? null;
}
export function getPool(targetSpec, owner, sourceCard, condition = {}, context = {}) {
    ...
    if (type === "hand") {
        ...
        if (side === "ally" || side === "self")
            pool = myBoard || [];
        else if (side.startsWith("enemy") || side === "opp" || side === "opponent")
            pool = oppBoard || [];
        else if (side === "any" || side === "both" || side === "all")
            pool = [...(myBoard || []), ...(oppBoard || [])];
        else
            pool = myBoard || [];
    }
    // Type filter
    if (type === "follower")
        ...
        pool = pool.filter(c => c?.type === "Amulet");
    // Condition filters
    // Default: exclude self, unless explicitly allowed
    const allowSelf = (condition && (condition.not_self === false || condition.include_self === true));
    ...
    if (condition.attack_lte != null) {
        ...
        if (lim != null)
            pool = pool.filter(c => c?.type === "Follower" && (Number(c.attack) || 0) <= lim);
    }
    ...
    if (condition.defense_lte != null) {
        ...
        if (lim != null)
            pool = pool.filter(c => c?.type === "Follower" && (Number(c.defense) || 0) <= lim);
    }
...
export function highlightSelectable(cards) {
...
export function handleSelect(eff, owner, sourceCard, effectsQueue, context = {}) {
    ...
    if (typeof window !== "undefined" && window.__BOT_AUTO_TARGETING__ === true) {
        ...
        if (mustFirst.length) {
            ...
            for (const c of pool) {
                if (picks.length >= effectiveCount)
                    break;
                if (set.has(c?.uid))
                    picks.push(c);
            }
