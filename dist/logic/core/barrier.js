...
export function dealDamage(target, amount, source = null) {
    ...
    try {
        ...
        const currentActive = state.activePlayer || (state.isBlueTurn ? "blue" : "red");
    ...
    if (countsAsDamaged && target?.type === "Follower" && target.defense > 0) {
        const owner = state.blueBoard.includes(target) ? "blue" :
            state.redBoard.includes(target) ? "red" : null;
        // Re-calculating owner here? 
        if (owner && (state.activePlayer || (state.isBlueTurn ? "blue" : "red")) === owner) {
...
export function applyBuff(card, attackBuff, defenseBuff) {
    ...
    if (!card.base_attack)
        card.base_attack = atk;
    if (!card.base_defense)
        card.base_defense = def;
    // Apply buffs
...
export function setPotentialFromBasePlus(card, atkDelta = 0, defDelta = 0) {
    ...
    if (card.base_attack == null)
        card.base_attack = (parseInt(card.attack, 10) || 0) - atkDelta;
    if (card.base_defense == null)
        card.base_defense = (parseInt(card.defense, 10) || 0) - defDelta;
