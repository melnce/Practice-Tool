...
function enemyOf(player) { return player === "blue" ? "red" : "blue"; }
function hasWardOn(board) { return board.some(c => c && c.type === "Follower" && c.hasWard); }
function canTargetFollower(defender, defenderBoard) {
    if (!defender || defender.type !== "Follower")
        return false;
    if (defender.hasAmbush)
        return false;
    if (defender.hasIntimidate)
        return false; // NEW: Intimidate = cannot be attacked
    if (hasWardOn(defenderBoard) && !defender.hasWard)
        return false;
    return true;
}
function effectiveAtk(card) {
    return Math.max(0, parseInt(card?.attack, 10) || 0);
}
/* attack counter + flags */
function spendAttack(attacker) {
    if (attacker.attacks_left == null) {
        const per = Number.isFinite(attacker.attacks_per_turn) ? attacker.attacks_per_turn : 1;
    ...
    if (owner === "blue")
        state.blueAnyAllyAttackedThisTurn = true;
    else
        state.redAnyAllyAttackedThisTurn = true;
...
function isAttackForbidden(card) {
    if (!card)
        return false;
...
function recomputeAttackFlags(card) {
    const eligible = !!(card.hasStorm || !card.justPlayed || card.hasRush);
    const swingsLeft = (card.attacks_left ?? 0) > 0;
    card.can_attack = eligible && swingsLeft;
    card.isRush = !!(card.justPlayed && card.hasRush && !card.hasStorm);
}
function stripAmbushOnSelfAttack(attacker) { if (attacker.hasAmbush)
    attacker.hasAmbush = false; }
...
function hasPiercingOne(attacker) { return attacker?.evoType === "super" || !!attacker?.hasPiercing; }
/* --------------------------- follower vs follower --------------------------- */
function _attackFollowerCore(attackerIdx, defenderIdx, attackerPlayer, defenderPlayer) {
    ...
    if (!attacker || !defender)
        return;
    if (attacker.type !== "Follower" || defender.type !== "Follower")
        return;
    if (!attacker.can_attack || (attacker.attacks_left ?? 1) <= 0)
        return;
    if (isAttackForbidden(attacker))
        return;
    if (!canTargetFollower(defender, defenderBoard))
        return;
    ...
    if (attacker.attacks_left == null) {
        attacker.attacks_left = Number.isFinite(attacker.attacks_per_turn)
            ? attacker.attacks_per_turn
            : 1;
    }
    // Normalize stats after pre-damage triggers
    attacker.attack = parseInt(attacker.attack) || 0;
    attacker.defense = parseInt(attacker.defense) || 0;
    defender.attack = parseInt(defender.attack) || 0;
    defender.defense = parseInt(defender.defense) || 0;
    ...
    if (hasCardTrigger(attacker, "follower_strike", "board")) {
        ...
        if (!stillThere || stillThere !== defender || defender.defense <= 0) {
    ...
    if (hasPiercingOne(attacker) && (parseInt(defender.defense) || 0) <= 0) {
        const hpKey = leaderHPKey(defenderPlayer);
        state[hpKey] = Math.max(0, state[hpKey] - 1);
...
export function attackFollower(attackerIdx, defenderIdx, attackerPlayer, defenderPlayer) {
    const meta = { attackerIdx, defenderIdx, attackerPlayer, defenderPlayer };
    return doAction("Attack Follower", () => _attackFollowerCore(attackerIdx, defenderIdx, attackerPlayer, defenderPlayer), meta, { autoRender: false });
}
/* ----------------------------- follower -> leader ---------------------------- */
function _attackLeaderCore(attackerIdx, attackerPlayer, defenderPlayer) {
    ...
    if (attacker.hasRush && attacker.justPlayed && !attacker.hasStorm)
        return;
    if (!attacker.can_attack || (attacker.attacks_left ?? 1) <= 0)
        return;
    if (isAttackForbidden(attacker))
        return;
    if (hasWardOn(defenderBoard))
        return;
    stripAmbushOnSelfAttack(attacker);
    if (attacker.attacks_left == null) {
        attacker.attacks_left = Number.isFinite(attacker.attacks_per_turn)
...
export function attackLeader(attackerIdx, attackerPlayer, defenderPlayer) {
    const meta = { attackerIdx, attackerPlayer, defenderPlayer };
    return doAction("Attack Leader", () => _attackLeaderCore(attackerIdx, attackerPlayer, defenderPlayer), meta, { autoRender: false });
}
/* ------------------------------ drag-drop hook ------------------------------ */
export function handleDropOnLeader(attackerIdx, attackerPlayer) {
