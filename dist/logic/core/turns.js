 ...
 */
function refreshBoardForNewTurn(board) {
    board.forEach(card => {
        if (card?.type !== "Follower")
            return;
        // Reset swing counters for multi-attack followers
        const perTurn = Number.isFinite(card.attacks_per_turn) ? card.attacks_per_turn : 1;
        card.attacks_per_turn = perTurn;
        card.attacks_left = perTurn;
        // Respect summoning sickness unless Rush/Storm
        card.can_attack = (!card.justPlayed) || card.hasRush || card.hasStorm;
        // Legacy/UI flag
...
function clearTempHandCostMods(endedPlayer) {
    const hand = endedPlayer === "blue" ? state.blueHand : state.redHand;
    for (const card of hand) {
        const delta = parseInt(card.temp_cost_mod_until_eot) || 0;
        if (delta !== 0) {
            card.cost_mod = (parseInt(card.cost_mod) || 0) - delta;
            delete card.temp_cost_mod_until_eot;
        }
    }
}
    ...
    for (const { owner, board } of sides) {
        for (const card of board) {
            if (!card || card.type !== 'Follower')
                continue;
            if (!card.hasBleed || !card.bleed)
                continue;
            const toLeader = Number(card.bleed.toLeader || 0);
            const toSelf = Number(card.bleed.toSelf || 0);
            if (toLeader > 0) {
                if (owner === "blue") {
            ...
            if (toSelf > 0)
                dealDamage(card, toSelf); // the follower itself
        }
    }
}
...
export function endTurnBlue() {
    ...
    resetEngageFlagsAtTurnStart("red");
    {
        ...
        if (redStartFx.length)
            runEffects([...redStartFx], "red");
    }
    {
        const redCrestFx = tickCrests("red");
        if (redCrestFx.length)
            runEffects([...redCrestFx], "red");
    }
    cleanupDead();
...
export function endTurnRed() {
    ...
    if (state.redBoostPending) {
        if (state.roundCount <= 5)
            state.redBoostUsedEarly = true;
        else
            state.redBoostUsedLate = true;
    ...
    resetEngageFlagsAtTurnStart("blue");
    {
        ...
        if (blueStartFx.length)
            runEffects([...blueStartFx], "blue");
    }
    {
        const blueCrestFx = tickCrests("blue");
        if (blueCrestFx.length)
            runEffects([...blueCrestFx], "blue");
    }
    cleanupDead();
...
function handleCountdowns(owner) {
    const board = owner === "blue" ? state.blueBoard : state.redBoard;
    board.forEach(card => {
        if (card?.type === "Amulet" && card.hasCountdown) {
            card.countdown = (Number(card.countdown) || 0) - 1;
        }
    });
}
function clearSummoningSickness(board) {
    board.forEach(card => {
        if (card?.type === "Follower") {
...
function clearCantAttackUntilOpponentEOT(board) {
    board.forEach(card => {
        if (!card || card.type !== "Follower")
            return;
        if (!card.cantAttackUntilOpponentEOT)
            return;
