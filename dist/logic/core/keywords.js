...
export function applyKeyword(card, keywordName, options) {
    ...
    if (keywordHandler) {
        ...
        const isTempCantAttack = (key === "cant_attack") && (options?.until_opponent_eot || options?.expires_on_turn);
        if (!__initializingKeywords && !isTempCantAttack) {
            ...
            const hasAlready = card.keywords.some(k => (typeof k === "string" && k.toLowerCase() === key) ||
                (k && typeof k === "object" && String(k.name || "").toLowerCase() === key));
            if (!hasAlready)
                card.keywords.push(key);
        }
    }
    else {
...
export function handleRemoveKeyword(eff, owner, explicitTargets) {
    ...
    for (const target of targets) {
        ...
        if (keywordToRemove === "ward")
            target.hasWard = false;
        else if (keywordToRemove === "rush")
            target.hasRush = false;
        else if (keywordToRemove === "storm")
            target.hasStorm = false;
        else if (keywordToRemove === "bane")
            target.hasBane = false;
        else if (keywordToRemove === "intimidate")
            target.hasIntimidate = false;
        else if (keywordToRemove === "drain")
            target.hasDrain = false;
        else if (keywordToRemove === "lastwords")
            target.hasLastWords = false;
        else if (keywordToRemove === "ambush")
