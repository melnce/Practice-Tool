...
function injectDescAndBadge(card, htmlChunk, badgeLines) {
    ...
    const parts = [];
    if (card.__descBase)
        parts.push(card.__descBase);
    if (card.__descChunks.length)
        parts.push(...card.__descChunks);
    ...
    button.addEventListener('click', () => {
        ...
        doAction("Confirm Targets", () => {
            ...
            if (eff.op === "fuse_finalize_fortifier") {
                if (targets?.length)
                    opFinalizeFortifierFuse(owner, eff.initiator_uid, targets);
            }
            else if (eff.op === "fuse_finalize_gear_multi") {
                if (targets?.length)
                    opFinalizeGearMulti(owner, eff.initiator_uid, targets, eff.result_name);
            }
...
function isSuperProtected(card, owner) {
    if (!card || card.type !== "Follower")
        return false;
    const isAlly = (owner === "blue" && state.blueBoard.includes(card)) ||
    ...
    if (eff.op === "damage") {
        ...
        for (const target of targets) {
            ...
            if (sourceCard && String(sourceCard.name).toLowerCase() === "flight of icarus") {
                injectDescAndBadge(target, `<span style="color: orange;">Rush<br>Last Words: Draw a card</span>`, ["Rush", "Last Words: Draw a card"]);
            }
            ...
            if (sourceCard && String(sourceCard.name).toLowerCase() === "carnelia, ember of darkness") {
                injectDescAndBadge(target, `<span style="color: orange;">Ward<br>Can't be destroyed by abilities</span>`, ["Ward", "Can't be destroyed by abilities"]);
            }
        }
    }
        ...
        if (targets.length > 0) {
            if (owner === "blue")
                state.blueShadows += targets.length;
            else
                state.redShadows += targets.length;
        }
        ...
        if (!target) {
            ...
            logEvent("effectDamage", {
                ...
                op: "damage_follower_or_leader",
                amount: eff.amount,
                target: target ? { type: target.type, name: target.name, uid: target.uid }
        ...
        if (sourceCard && target.uid === sourceCard.uid)
            return; // "not_self" safety
        if (target.hasEvolved)
            return; // must be unevolved
