...
function notifyLootPlayed(owner, sourceCard) {
    if (!sourceCard)
        return;
    const isLoot = Array.isArray(sourceCard.tribes) &&
    ...
    while (effects.length) {
        ...
        switch (eff.op) {
            case "add_counter":
                ...
                break;
            case "add_selected_copy_to_hand": {
                const t = context?.selectedCard || (context?.targets?.[0] || null);
                if (!t || !t.name)
                    break;
                handleAddToHand({ count: eff.count ?? 1, name: t.name }, owner);
                break;
            }
            case "add_shadows":
                handleAddShadows(eff, owner);
                break;
            case "add_to_hand":
                handleAddToHand(eff, owner);
                break;
            case "amulet_count_gate": {
                const next = amuletCountGate(owner, eff) ? (eff.effects || []) : (eff.else_effects || []);
                if (next.length)
                    effects.unshift(...next);
                break;
            }
            case "attacks_per_turn":
                applyAttacksPerTurn(eff, sourceCard);
                break;
            case "banish":
                if (handleBanishTargeted(eff, owner, effects) === "pending")
                    return;
                break;
            case "banish_all_enemy_copies": {
                const target = context?.selectedCard || (context?.targets?.[0] || null);
                if (target) {
                    handleBanishAllEnemyCopies(owner, target);
                }
                break;
            }
            case "banish_duplicates_from_deck":
                handleBanishDuplicatesFromDeck(owner);
                return "done";
            case "banish_random":
                handleBanishRandom(eff, owner);
                break;
            case "banish_self":
                handleBanishSelf(sourceCard);
                break;
            ...
            case "buff": {
                const res = handleBuff(eff, owner, sourceCard, [], context);
                if (res === "pending")
                    return res;
                break;
            }
            case "buff_hand_class":
                handleBuffHandClass(eff, owner);
                break;
            case "buff_last_added_to_hand":
                handleBuffLastAddedToHand(eff, owner);
                break;
            case "buff_self":
                handleBuffSelf(sourceCard, eff);
                break;
            case "buff_hand_tribe":
                handleBuffHandTribe(eff, owner);
                break;
            case "chaos_split_damage":
                import('../effects/cards/runecraft/chaos.js').then(({ handleChaosSplitDamage }) => { handleChaosSplitDamage(owner, sourceCard); });
                break;
            case "choose":
                if (handleChoose(eff, owner, sourceCard, effects) === "pending")
                    return;
                break;
            case "choose_bonus_add": {
                if (owner === "blue")
                    state.blueChooseBonus = (state.blueChooseBonus || 0) + (parseInt(eff.amount ?? 1, 10) || 0);
                else
                    state.redChooseBonus = (state.redChooseBonus || 0) + (parseInt(eff.amount ?? 1, 10) || 0);
                break;
            }
            case "combo_add":
                handleComboAdd(owner, eff);
                break;
            case "combo_gate":
                if (handleComboGate(owner, eff)) {
                    effects.unshift(...(eff.effects || []));
                }
                else {
                    effects.unshift(...(eff.else_effects || []));
                ...
                break;
            case "combo_repeat_buff": {
                const res = handleComboRepeatBuff(eff, owner, sourceCard, effects, context);
                if (res === "pending")
                    return res;
                break;
            }
            ...
            case "crest_pay_counter": {
                ...
                if (chain.length)
                    effects.unshift(...chain);
                break;
            }
            case "damage":
                if (handleDamage(eff, owner, sourceCard, effects, context) === "pending")
                    return;
                break;
            case "damage_all":
                handleDamageAll(eff, owner, sourceCard);
                break;
            case "damage_all_by_allied_golems":
                handleDamageAllByAlliedGolems(eff, owner);
                break;
            case "damage_enemy_leader_by_other_allies":
                handleDamageEnemyLeaderByOtherAllies(owner, sourceCard);
                break;
            case "damage_follower_or_leader":
                if (handleDamageFollowerOrLeader(eff, owner, sourceCard, effects) === "pending")
                    return;
                break;
            case "damage_highest_defense":
                handleDamageHighestDefense(eff, owner, sourceCard);
                break;
            case "damage_random":
                handleDamageRandom(eff, owner);
                break;
            case "damage_random_selected_defense":
                handleDamageRandomSelectedDefense(eff, owner, sourceCard, effects, context);
                break;
            case "damage_self": {
            ...
            case "damage_split_all_enemies":
                handleDamageSplitAllEnemies(eff, owner, sourceCard);
                break;
            case "damage_split_fixed":
                handleDamageSplitFixed(eff, owner);
                break;
            case "damage_split_sequential":
                handleDamageSplitSequential(eff, owner);
                break;
            case "destroy":
                if (handleDestroy(eff, owner, effects, context, sourceCard) === "pending")
                    return;
                break;
            case "destroy_all":
                handleDestroyAll(eff, owner, sourceCard, context);
                break;
            case "destroy_allied_amulets_then_damage":
                handleDestroyAlliedAmuletsThenDamage(owner);
                break;
            ...
            case "destroy_highest":
                handleDestroyHighest(eff, owner);
                break;
            case "destroy_random":
                handleDestroyRandom(eff, owner, context);
                break;
            case "destroy_random_other_allies":
                handleDestroyRandomOtherAllies(owner, sourceCard, context);
                break;
            case "destroy_self":
                handleDestroySelf(sourceCard);
                cleanupDead();
                break;
            case "destroy_then": {
            ...
            case "discard_all_except_named":
                handleDiscardAllExceptNamed(eff, owner);
                break;
            case "discard_select_hand":
                if (handleDiscardSelectHand(eff, owner, effects) === "pending")
                    return;
                break;
            case "double_stats_allies": {
                return import("../effects/doubleStats.js").then(({ doubleStatsAllies }) => { doubleStatsAllies(owner); });
            }
            case "dragonsign":
                handleDragonSign(owner);
                break;
            case "draw":
                handleDraw(eff, owner);
                break;
            case "draw_all_named_with_keyword":
                handleDrawAllNamedWithKeyword(eff, owner);
                break;
            case "draw_combo_follower":
                handleDrawComboFollower(eff, owner);
                break;
            case "draw_filtered":
                handleDrawFiltered(eff, owner);
                break;
            case "draw_named":
                handleDrawNamed(eff, owner);
                break;
            case "draw_opponent":
                handleDrawOpponent(eff, owner);
                break;
            case "dynamic_buff_self":
                handleDynamicBuffSelf(sourceCard, eff, owner);
                break;
            case "dynamic_heal_leader":
                handleDynamicHealLeader(owner, eff);
                logEvent("healLeader", { owner, amount: eff.amount });
                break;
            case "earth_rite":
                if (consumeEarthSigils(owner, eff.cost || 1))
                    effects.unshift(...(eff.effects || []));
                break;
            ...
            case "evolve_self":
                ...
                logEvent("evolve", { owner, card: sourceCard?.name });
                break;
            case "fill_congregant_copies":
                handleFillCongregantCopies(owner, sourceCard);
                break;
            case "follower_strike_destroy":
                if (sourceCard && context?.defender) {
                    resolveDestroy(context.defender, owner);
                    cleanupDead();
                }
                break;
            case "fuse_finalize_fortifier":
            ...
            case "fuse_start":
                ...
                break;
            case "gain_crest":
                handleGainCrest(eff, owner);
                logEvent("gainCrest", { owner, crest: eff.name });
                break;
            case "gain_max_pp": {
                const amt = parseInt(eff.amount ?? 1) || 1;
            ...
            case "halve_deck_cost":
                handleHalveDeckCost(owner);
                break;
            case "hand_count_gate": {
                const pass = handCountGate(owner, eff);
                const next = pass ? (eff.effects || []) : (eff.else_effects || []);
                if (next.length)
                    effects.unshift(...next);
                break;
            ...
            case "heal_leader": {
                handleHealLeader(owner, eff);
                logEvent("healLeader", { owner, amount: eff.amount });
                const targetOwner = (eff.player || "self") === "self" ? owner : (owner === "blue" ? "red" : "blue");
            ...
            case "himeka_crest_effect":
                handleHimekaCrestEffect(owner);
                break;
            case "increase_countdown": {
                const amt = Number(eff.amount ?? 1);
                handleIncreaseCountdown(owner, amt);
                break;
            }
            case "increase_opponent_hand_cost_eot": {
                const amt = parseInt(eff.amount ?? 1) || 1;
                applyTempOpponentHandCostMod(owner, amt);
                break;
            }
            case "juno_damage":
                if (handleJunoDamage(eff, owner, sourceCard, effects) === "pending")
                    return;
                break;
            case "keyword": {
                const merged = { ...(context || {}), sourceCard };
                if (handleKeyword(eff, owner, effects, merged) === "pending")
                    return;
                break;
            }
            case "keyword_self": {
                const target = sourceCard || (Array.isArray(state.lastSummoned) ? state.lastSummoned[0] : null);
                handleKeywordSelf(target, eff);
                break;
            }
            case "kuon_enhance":
                import('../effects/cards/runecraft/kuon.js').then(({ handleKuonEnhance }) => { handleKuonEnhance(owner); });
                break;
            case "leader_barrier": {
                handleLeaderBarrierOp(owner, eff);
                break;
            }
            case "modify_cost":
                handleModifyCost(eff, owner, sourceCard, context);
                break;
            case "modify_cost_pool":
                handleModifyCostPool(eff, owner, sourceCard);
                break;
            case "necromancy_gate":
                if (hasNecromancy(owner, eff.cost || 1)) {
                    const cost = eff.cost || 1;
                    spendShadows(owner, cost);
                    logEvent("necromancySpend", { owner, cost });
                    effects.unshift(...(eff.effects || []));
                }
                break;
            ...
            case "no_duplicates_in_deck_gate": {
                if (hasNoDuplicatesInDeck(owner)) {
                    effects.unshift(...(eff.effects || []));
                }
                break;
            }
            ...
            case "recover_pp":
                handleRecoverPP(owner, eff);
                break;
            case "repeat_effect":
                handleRepeatEffect(eff, owner, sourceCard, effects);
                break;
            case "reduce_cost": {
                const target = (context && (context.targetCard || context.selectedCard)) || sourceCard;
                handleReduceCost(target, eff);
                break;
            }
            case "reduce_cost_self":
                handleReduceCostSelf(sourceCard, eff);
                break;
            case "reduce_countdown":
                handleReduceCountdown(sourceCard, eff);
                break;
            case "reduce_deck_followers_cost": {
                const amt = parseInt(eff.amount ?? 1) || 1;
                reduceDeckFollowersCost(owner, amt);
                break;
            }
            case "remove_keyword":
                if (handleRemoveKeyword(eff, owner) === "pending")
                    return;
                break;
            case "replace_deck":
                handleReplaceDeck(owner, eff);
                break;
            case "replace_deck_with_set_minus": {
                import("@logic/effects/deck.js").then(({ replaceDeckWithSetMinus }) => { replaceDeckWithSetMinus(owner, eff).then(() => render()); });
                break;
            }
            case "restore_full_defense_self":
                handleRestoreFullDefenseSelf(sourceCard, context);
                break;
            case "restore_self_and_heal_leader":
                handleRestoreSelfAndHealLeader(owner, sourceCard);
                break;
            case "return_hand_to_deck":
                if (handleReturnHandToDeck(eff, owner, effects) === "pending")
                    return;
                break;
            case "return_to_hand":
                if (handleReturnToHand(eff, owner, sourceCard, effects) === "pending")
                    return;
                break;
            case "select": {
            ...
            case "select_evolve_golem":
                if (handleSelectEvolveGolem(eff, owner, sourceCard, effects) === "pending")
                    return;
                break;
            ...
            case "select_hand_summon_artifact_copies_eot_destroy":
                logEvent("summon", { owner, op: eff.op, status: "pending_selection" });
                if (handleSelectHandSummonArtifactCopiesEOT(eff, owner, effects) === "pending")
                    return;
                break;
            case "set_attack_to": {
                const res = handleSetAttackTo(eff, owner, sourceCard, effects, context);
                if (res === "pending")
                    return res;
                break;
            }
            case "set_deckout_victory": {
                const enable = eff.enabled !== false;
                if (owner === "blue")
                    state.deckoutWinsBlue = enable;
                else
                    state.deckoutWinsRed = enable;
                break;
            }
            case "set_max_hp":
                handleSetMaxHP(eff, owner);
                break;
            case "spellboost_hand":
                spellboostHand(owner, eff);
                break;
            case "spellboost_target":
                ...
                break;
            case "start_fortifier_fuse": {
                const res = sourceCard ? startFortifierFuse(owner, sourceCard) : null;
                if (res === "pending")
                    return res;
                logEvent("fuse", { owner, op: eff.op, source: sourceCard?.name });
                break;
            }
            case "start_fuse_from_card": {
            ...
            case "set_cost_self":
                handleSetCostSelf(sourceCard, eff);
                break;
            case "stormy_blast_counter":
                import('../effects/cards/runecraft/stormyBlast.js').then(({ handleStormyBlastCounter }) => { handleStormyBlastCounter(sourceCard); });
                break;
            case "stormy_blast_damage":
                import('../effects/cards/runecraft/stormyBlast.js').then(({ handleStormyBlastDamage }) => { return handleStormyBlastDamage(eff, owner, sourceCard, effects); });
                return;
            case "super_evo_gate":
                if (handleSuperEvoGate(owner)) {
                    effects.unshift(...(eff.effects || []));
                }
                break;
            case "super_evolve_ally": {
                import("@logic/evolveUtils.js").then(({ superEvolveAllyFromContext }) => { superEvolveAllyFromContext(owner, sourceCard, context); });
                break;
            }
            ...
            case "super_evolve_self":
                ...
                break;
            case "super_evolved_self_gate": {
                const isSuper = sourceCard && sourceCard.evoType === "super";
            ...
            case "summon_destroyed_amulet_highest_base_cost":
                handleSummonDestroyedAmuletHighestBaseCost(owner);
                logEvent("summon", { owner, name: "(highest cost destroyed amulet)" });
                break;
            ...
            case "summon_named":
                summonNamed(eff, owner);
                logEvent("summon", { owner, name: eff.name });
                break;
            case "summon_named_enemy": {
                const foe = (owner === "blue") ? "red" : "blue";
                summonNamed({ op: "summon_named", name: eff.name, count: eff.count || 1 }, foe);
                logEvent("summon", { owner: foe, name: eff.name });
                break;
            }
            case "summon_random_from_deck":
                summonRandomFromDeck(eff, owner);
                logEvent("summon", { owner, name: "(random from deck)" });
                break;
            case "transform": {
            ...
            case "transform_in_hand":
                handleTransformInHand(eff, owner);
                break;
            case "transform_random_spell_in_hand": {
                transformRandomSpellInHand(owner, eff.into || "Ersatz Elimination");
                break;
            }
            case "transform_self_if_spellboost_at_least": return;
            case "william_counter":
                import('../effects/cards/runecraft/william.js').then(({ handleWilliamCounter }) => { handleWilliamCounter(sourceCard); });
                break;
            case "william_damage_all": {
                const x = (sourceCard?.currentWilliamDamage ?? sourceCard?.spellboostCount ?? 0) | 0;
