...
const STRIP_KEYS = new Set([
    ...
    "__mulliganSelectable", "__mulliganSelected",
    "__dragging", "__hover", "__glow", "__ui",
    "potential_attack", "potential_defense",
    "_death_snapshot", "draggingId", "dropTargetId",
    "element", "el", "dom"
]);
function isPlainObject(v) {
    return v && typeof v === "object" && !Array.isArray(v) &&
        !(v instanceof Date) && !(v instanceof Set) &&
        !(v instanceof Map) && !(v instanceof ArrayBuffer) &&
        !(ArrayBuffer.isView(v));
}
function normalizeForStable(value, seen) {
    if (value == null)
        return value;
    // Avoid cycles
    ...
    if (t === "bigint")
        return value.toString(); // keep deterministically
    ...
    if (value instanceof Map) {
        const obj = {};
        const entries = Array.from(value.entries()).map(([k, v]) => [String(k), normalizeForStable(v, seen)]);
    ...
    if (isPlainObject(value)) {
        const out = {};
        const keys = Object.keys(value).filter(k => !STRIP_KEYS.has(k)).sort();
        for (const k of keys) {
...
export function logEvent(type, details = {}) {
    if (globalThis.HEADLESS)
        return; // <-- Add this line to silence logs in headless mode
    const id = ++_id;
    ...
    const entry = { ...meta, details, stateHash: null };
    _log.push(entry);
    // Enforce cap
    if (_log.length > _maxEntries)
        _log.splice(0, _log.length - _maxEntries);
    ...
    computeStateHash().then(h => {
        ...
        if (_mirrorToConsole) {
            ...
            console.log(`[LOG #${id}] ${type} | turn=${meta.turn} owner=${meta.activePlayer} | hash=${h}`, entry);
        }
    });
    return id;
}
...
if (typeof window !== "undefined") {
    window._gameLog = _log;
    window.downloadGameLog = downloadLogs;
    window.clearGameLog = clearLogs;
    window.computeStateHash = computeStateHash;
    window.setGameLogSession = setSessionTag;
}
