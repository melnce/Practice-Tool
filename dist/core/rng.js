...
export function setSeed(seedLike) {
...
export function shuffleInPlace(arr, rng = rand) {
    const n = arr.length;
    for (let i = n - 1; i > 0; i--) {
        const from = i + 1;
...
export function makeUid(prefix = "uid_") {
    const from = 36 ** 8;
...
export function setRngSnapshot(snap) {
    if (!snap || typeof snap.seed === "undefined")
        return;
    setSeed(snap.seed);
    // advance to the same cursor position
    for (let i = 0; i < (snap.cursor | 0); i++)
        _rng();
