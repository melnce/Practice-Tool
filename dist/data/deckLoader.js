...
async function fetchDeck(deckId) {
    ...
    if (!res.ok)
        throw new Error(`Deck not found at ${url}`);
    const obj = await res.json();
    obj.__deckFile = file; // keep filename for later checks
    return obj;
}
export async function loadBlueDeck(deckName) {
...
export async function loadRedDeck(deckName) {
    ...
    state.redMaxPP = 1;
    for (let i = 0; i < 4; i++)
        drawCard(state.redHand, state.redDeck);
