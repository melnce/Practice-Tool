...
function valueFromEntry(entry) {
  ...
  if (typeof entry === 'string') return entry.replace(/^decks\//, '').replace(/\.json$/i, '');
  if (entry && typeof entry === 'object') {
    if (entry.file) return entry.file.replace(/^decks\//, '').replace(/\.json$/i, '');
    if (entry.name) return entry.name.replace(/^decks\//, '').replace(/\.json$/i, '');
  }
  return 'example_deck';
}
function labelFromEntry(entry) {
  if (typeof entry === 'string') return pretty(entry);
  if (entry && typeof entry === 'object') return pretty(entry.label || entry.name || entry.file || '');
  return 'example_deck';
}
