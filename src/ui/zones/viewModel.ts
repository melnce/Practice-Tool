...
export function createCardViewModel(
    ...
    const availablePP = ctx.isHand ? (ctx.isBlueHand ? state.bluePP : state.redPP) : 0;
    ...
    if (isFollower) {
        ...
        if (ctx.isHand) {
            ...
            const baseAttack = card.base_attack != null ? Number(card.base_attack) : (Number(card.attack) || 0);
            const baseDefense = card.base_defense != null ? Number(card.base_defense) : (Number(card.defense) || 0);
    ...
    const isMyBoard = (ctx.containerId === "blueBoard" && isBlue) || (ctx.containerId === "redBoard" && !isBlue);
    ...
    if (ctx.isBoard && isFollower && card.can_attack && !card.hasAttacked && isMyBoard && isMyTurn) {
    ...
    if (card.type === "Amulet") {
        ...
        if (!ctx.isMulligan && ctx.isBoard && card.hasEngage) {
            const ks = card.keywordState;
            const enoughPP = isBlue ? state.bluePP >= engageCost : state.redPP >= engageCost;
            ...
            const isMyTurn = (isBlue && state.isBlueTurn) || (!isBlue && !state.isBlueTurn);
    ...
    return {
        ...
        countdown: (card.hasCountdown && Number.isFinite(Number(card.countdown))) ? Number(card.countdown) : null,
        ...
        engageCost,
        isSelected,
        isSelectable: ctx.isMulligan ? !!card.__mulliganSelectable : !!card.__uiSelectable,
