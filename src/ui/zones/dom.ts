...
export function renderCardDOM(vm: CardViewModel, elementId: string, tooltipContainer: HTMLElement | null): HTMLElement {
    ...
    // Stats
    const topLeft = createElement("div", "card-stats top-left", String(vm.shownCost));
    ...
    if (card.type === "Follower") {
        ...
        if (vm.countdown !== null) {
            ...
            if (card.counters && typeof card.counters === "object") {
                const entries = Object.entries(card.counters).filter(([, v]) => Number.isFinite(Number(v)));
                if (entries.length) {
    ...
    if (vm.spellboostCount !== null && vm.spellboostCount > 0) {
        const sb = createElement("div", "spellboost-badge", String(vm.spellboostCount));
