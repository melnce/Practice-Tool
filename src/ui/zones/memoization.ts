...
function isStateSame(prev: StateArgs, state: GameState, ctx: ZoneContext): boolean {
    ...
    if (prev.boardLens !== `${state.blueBoard.length}|${state.redBoard.length}`) return false;
...
export function getMemoizedViewModel(
    ...
    if (cached) {
        if (isStateSame(cached.lastStateArgs, state, ctx) &&
