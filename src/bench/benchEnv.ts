...
import { getBoard, getHand, getPP, getHP, opponentOf } from "../core/playerHelpers.js";
...
export const benchTiming: BenchTiming = {
    ...
    summary() {
        ...
        const byActionType: Record<string, { count: number; ms: number; avgUs: number }> = {};
        for (const key of ["PLAY_CARD", "ATTACK_FOLLOWER", "ATTACK_LEADER", "END_TURN"] as const) {
            const b = this[key];
            byActionType[key] = {
                count: b.count,
                ms: b.ms,
        ...
        const totalStageMs = Object.values(this.stages).reduce((a, b) => a + b.ms, 0);
        const byStage: Record<string, { count: number; ms: number; pct: number; avgUs: number }> = {};
...
export function getLegalActions(): BenchAction[] {
    ...
    for (let i = 0; i < hand.length; i++) {
        ...
        if ((card.type === "Follower" || card.type === "Amulet") && myBoard.length >= 5) {
            continue;
        }
