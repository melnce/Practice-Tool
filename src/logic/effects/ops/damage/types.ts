...
export function validateUnifiedSpec(spec: UnifiedDamageSpec): string[] {
    ...
    if (spec.count !== undefined && spec.distribution !== "random_hits") {
        warnings.push(`'count' field only applies to distribution='random_hits', not '${spec.distribution}'`);
    }
    ...
    if (spec.stat && spec.distribution !== "by_stat") {
        warnings.push(`'stat' field only applies to distribution='by_stat', not '${spec.distribution}'`);
    }

    // spill_to_leader only valid for split_sequential
    if (spec.spill_to_leader !== undefined && spec.distribution !== "split_sequential") {
        warnings.push(`'spill_to_leader' field only applies to distribution='split_sequential'`);
    }

    // include_leader only valid for random_hits
    if (spec.include_leader !== undefined && spec.distribution !== "random_hits") {
        warnings.push(`'include_leader' field only applies to distribution='random_hits'`);
    }
 ...
 * Applies defaults and derives distribution mode from legacy op names.
 */
export function normalizeToUnifiedSpec(
    ...
    if (eff.fallback_leader !== undefined || eff.can_target_leader !== undefined) {
        spec.fallback_leader = Boolean(eff.fallback_leader ?? eff.can_target_leader);
    }
    ...
    switch (op) {
        case "damage_all":
            spec.distribution = spec.distribution === "direct" ? "direct" : spec.distribution;
            break;

        case "damage_random":
            spec.distribution = "random_hits";
            spec.count = spec.count ?? (parseInt(String(eff.count ?? 1), 10) || 1);
            spec.include_leader = spec.include_leader ??
                (spec.target === "enemy" || spec.target === "enemy:all" || spec.target === "all");
            break;
