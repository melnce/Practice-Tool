...
export function handleDestroyHighest(eff, owner) {
  const pool = getPool(eff.target, owner).filter(
    (c) => c.type === "Follower" && (parseInt(c.defense, 10) || 0) > 0
  );
...
export function handleDestroyRandom(eff, owner, context = {}) {
  // 1) base pool
  let pool = getPool(
    eff.target || "enemy:follower",
    owner,
    null,
    eff.condition,
    context
  ).filter((c) => c && c.type === "Follower" && (parseInt(c.defense, 10) || 0) > 0);
