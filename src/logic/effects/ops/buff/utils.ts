...
export function filterBuffCandidates(pool: CardInstance[], eff: BuffOp, sourceCard: CardInstance | null): CardInstance[] {
    ...
    const rawTribes = eff.tribes
        ? (Array.isArray(eff.tribes) ? eff.tribes : [eff.tribes])
    ...
    if (eff.name_filter) {
        ...
        candidates = candidates.filter(c => String(c.name || "").toLowerCase() === want);
    }
    if (Array.isArray(eff.name_in) && eff.name_in.length) {
        const wants = new Set(eff.name_in.map((n: any) => String(n).toLowerCase()));
        candidates = candidates.filter(c => wants.has(String(c.name || "").toLowerCase()));
    }
    ...
    if (rawKW) {
        ...
        candidates = candidates.filter(c => wants.every((w: any) => hasKeyword(c, w)));
    }

    // Class filtering
    if (eff.condition && (eff.condition as any).class) {
        const wantClass = String((eff.condition as any).class).toLowerCase();
        candidates = candidates.filter(c => String(c.class || "").toLowerCase() === wantClass);
    }
