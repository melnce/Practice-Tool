...
export function handleSearch(
  ...
  for (const key in resolvedFilters) {
    if (
      typeof resolvedFilters[key] === "string" &&
      (resolvedFilters[key] as string).startsWith("{")
    ) {
      resolvedFilters[key] = resolveDynamicValue(resolvedFilters[key], {
  ...
  const selectedIndices = matchingIndices.sort((a, b) => b - a).slice(0, want);
