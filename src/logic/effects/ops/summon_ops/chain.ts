 ...
 */
export function makeChainDecayClone(
  prev: CardInstance,
  owner: Player,
): CardInstance {
  ...
  const prevBaseA = Number.isFinite(prev.base_attack)
    ? prev.base_attack!
    : prevAtk - (parseInt(String(prev.buffs?.attack)) || 0);
  const prevBaseD = Number.isFinite(prev.base_defense)
    ? prev.base_defense!
    : prevDef - (parseInt(String(prev.buffs?.defense)) || 0);
 ...
 * Logic generalized from Congregant of Unkilling to work for any card.
 */
export function handleFillBoardChainDecay(
  owner: Player,
  enteringCard: CardInstance | null,
) {
