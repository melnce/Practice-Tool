    ...
    special: (q, env) => {
        ...
        if (q.specialContext === "entering_follower") {
            if (env.context.enteringCard) {
                ...
                console.log(`%c[Targeting] Found entering_follower: ${env.context.enteringCard.name}`, 'color: green; font-weight: bold;');
...
export function resolveBasePool(query: TargetQuery, env: TargetingEnv): CardInstance[] {
