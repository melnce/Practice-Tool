...
export function applyFilters(pool: CardInstance[], query: TargetQuery, env: TargetingEnv): CardInstance[] {
    const cond = query.condition || {};
    let filtered = pool;

    // 1. Initial Type Filter (from parser, e.g. "ally:follower")
    if (query.typeFilter === "follower") filtered = filtered.filter(c => c?.type === "Follower");
    else if (query.typeFilter === "amulet") filtered = filtered.filter(c => c?.type === "Amulet");
    else if (query.typeFilter === "spell") filtered = filtered.filter(c => c?.type === "Spell");
    ...
    // 2. Self Exclusion
    const allowSelf = (query.side === "self") ||
        (cond.not_self === false || cond.include_self === true);
    ...
    if (cond.type) {
        ...
        filtered = filtered.filter(c => String(c?.type || "").toLowerCase() === typeFilter);
    }
    ...
    // 5. Tribe
    if (cond.tribe) {
        filtered = filtered.filter(c => Array.isArray(c?.tribes) && c.tribes!.includes(cond.tribe));
    }
    ...
    if (cond.is_super_evolved) {
        filtered = filtered.filter(c => c && c.type === "Follower" && c.evoType === "super");
    }

    // 9. Stat Filters
    if (cond.attack_lte != null) {
        const lim = toNum(cond.attack_lte);
        if (lim != null) filtered = filtered.filter(c => c?.type === "Follower" && (Number(c.attack) || 0) <= lim);
    }
