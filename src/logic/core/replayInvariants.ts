...
function countCardInZone(state: GameState, player: Player, zone: Zone, cardName: string): number {
...
export function invCardMoved(
    ...
    return {
        ...
        check({ initialState, finalState, meta, scenarioId }) {
            ...
            // Check From Zone
            const initialFromCount = countCardInZone(initialState, player, fromZone, cardName);
            const finalFromCount = countCardInZone(finalState, player, fromZone, cardName);
            ...
            // Check To Zone
            const initialToCount = countCardInZone(initialState, player, toZone, cardName);
            const finalToCount = countCardInZone(finalState, player, toZone, cardName);
 ...
 */
export function invLeaderHealth(idSuffix: string, player: Player, expected: number): ReplayInvariant {
    return {
        id: `leader_health_${player}_${idSuffix}`,
        description: `Assert ${player} leader health is ${expected}`,
        check({ scenarioId, finalState }) {
            const actual = player === "blue" ? finalState.blueLeaderHealth : finalState.redLeaderHealth;
            if (actual !== expected) {
                throw new ReplayInvariantError(
 ...
 */
export function invHandSize(idSuffix: string, player: Player, expected: number): ReplayInvariant {
    return {
 ...
 */
export function invBoardSize(idSuffix: string, player: Player, expected: number): ReplayInvariant {
    return {
        id: `board_size_${player}_${idSuffix}`,
        description: `Assert ${player} board size is ${expected}`,
        check({ scenarioId, finalState }) {
            const board = player === "blue" ? finalState.blueBoard : finalState.redBoard;
...
export function invCardCountInZone(
    idSuffix: string,
    params: { player: Player; zone: "hand" | "board" | "graveyard"; name: string; expected: number }
): ReplayInvariant {
 ...
 */
export function invEffectCount(idSuffix: string, params: { op: EffectOp; expected: number }): ReplayInvariant {
