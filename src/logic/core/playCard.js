...
function getEffectiveCost(card) {
  ...
  if (card.cost_mod != null) return (parseInt(card.cost, 10) || 0) + (parseInt(card.cost_mod, 10) || 0);
  if (card.costModified != null) return parseInt(card.costModified, 10) || 0;
...
function canCastSpell(card, player) {
  ...
  const needsHandReturn =
    effects.some(e => String(e.op).toLowerCase() === "return_hand_to_deck" && e.select);
...
function spellNeedsSpellboostTarget(card, player) {
  ...
  const hand = player === "blue" ? state.blueHand : state.redHand;
  return !hand.some(c =>
    Array.isArray(c.keywords) && c.keywords.some(k => {
...
function _playCardCore(fromHand, player, index) {
  ...
  if (isSpell) {
    ...
    if (list.length) runEffects([...list], player, spellCard, []);
    return render();
  }
