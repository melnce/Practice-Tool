...
function isAttackForbidden(card) {
  ...
  if (card.cantAttackIsTemporary && card.cantAttackUntilOpponentEOT) {
    ...
    const owner = ownerIsBlue ? "blue" : "red";
    if (state.activePlayer === owner) clearCantAttack(card);
  }
...
function stripAmbushOnSelfAttack(attacker) { if (attacker.hasAmbush) attacker.hasAmbush = false; }
...
function _attackFollowerCore(attackerIdx, defenderIdx, attackerPlayer, defenderPlayer) {
  ...
  if (!attacker.can_attack || (attacker.attacks_left ?? 1) <= 0) return;
