...
function _didRunOnceThisTurn(hostCard, turnNumber, key) {
  if (!key) return false;
  if (!hostCard.__onceByTurn) hostCard.__onceByTurn = Object.create(null);
...
export function fireTrigger(eventName, activePlayer, context = {}) {
    ...
    if (Array.isArray(crests)) {
      for (const crest of crests) {
        ...
        for (const cTrig of crestTriggers) {
          ...
          if (cTrig.once_per_turn) {
            const key = cTrig.once_key || `${cTrig.event || "any"}_once`;
            if (_didRunOnceThisTurn(crest, _turnToken, key)) continue;
            logEvent("trigger", { event: eventName, card: crest?.name });
            runEffects(list, activePlayer, crest, context);
            _markRanThisTurn(crest, _turnToken, key);
          } else {
  ...
  for (const { card, owner, source } of zones) {
    if (!Array.isArray(card?.triggers) || !card.triggers.length) continue;
    ...
    for (const trigger of card.triggers) {
      ...
      if (eventName === "self_damaged") {
        ...
        if (cond.still_alive && (parseInt(damaged.defense, 10) || 0) <= 0) continue;
        if (cond.own_turn && owner !== state.activePlayer) continue;
      ...
      if (typeof cond.attack_lte === "number" && (parseInt(card.attack, 10) || 0) > cond.attack_lte) continue;
      if (typeof cond.attack_gte === "number" && (parseInt(card.attack, 10) || 0) < cond.attack_gte) continue;
      if (typeof cond.defense_lte === "number" && (parseInt(card.defense, 10) || 0) > cond.defense_lte) continue;
      if (typeof cond.defense_gte === "number" && (parseInt(card.defense, 10) || 0) < cond.defense_gte) continue;
      if (cond.still_alive === true && (parseInt(card.defense, 10) || 0) <= 0) continue;
