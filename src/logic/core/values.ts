 ...
 */
export function resolveDynamicValue(val: string | number | undefined | null, context: ResolveContext = {}): number {
    ...
    // 2. {attacker.*} / {defender.*}
    if (s === "{attacker.attack}") return parseInt((context.attacker as any)?.attack || 0, 10) || 0;
    if (s === "{defender.defense}") return parseInt((context.defender as any)?.defense || 0, 10) || 0;
