...
function injectDescAndBadge(card: any, htmlChunk: string, badgeLines: string | string[]) {
    ...
    button.addEventListener('click', () => {
        ...
        doAction("Confirm Targets", () => {
            ...
            logEvent("targetsConfirmed", {
                ...
                targets: (pending?.targets || []).map((t: any) => ({ name: t?.name, uid: t?.uid, type: t?.type }))
            });
...
export function resolvePendingTarget(uid: string | "leader") {
    ...
    if (!pending.pool.some((p: any) => p.uid === uid)) {
    ...
    // --- Toggle selection ---
    const idx = pending.targets.findIndex((t: any) => t.uid === uid);
    if (idx !== -1) {
        // Unselect
    ...
    if (eff.op === "damage") {
        ...
        const discardedThisOp: CardInstance[] = [];
        ...
        if (discardedThisOp.length) {
            ...
            state.lastDiscardedCosts = discardedThisOp.map(c => parseInt(c.cost as any, 10) || 0);
        ...
        for (const dc of discardedThisOp) {
            if (Array.isArray((dc as any).on_discard) && (dc as any).on_discard.length) {
        ...
        for (const target of targets.filter(tribeOk)) {
            ...
            if (d < 0 && target?.type === "Follower") {
                ...
                if (targetOwner && debufferOwner) {
                    ...
                    fireTrigger("enemy_follower_defense_down", debufferOwner as any, { target });
                }
            }
        ...
        if (!target) {
            ...
            if (targetPlayer === "blue") {
                state.blueHP = Math.max(0, state.blueHP - (eff.amount as number));
            } else {
                state.redHP = Math.max(0, state.redHP - (eff.amount as number));
            }
        }
            ...
            dealDamage(target, eff.amount as number);
            cleanupDead();
        ...
        function findByUid(uid: string, ownerSide: Player) {
            ...
            for (const nestedEff of (eff.effects || [])) {
                if (nestedEff.op === 'set_stats') {
                    ...
                    if (nestedEff.attack !== undefined) {
                        const newAttack = parseInt(nestedEff.attack as any);
                    ...
                    if (nestedEff.defense !== undefined) {
                        const newDefense = parseInt(nestedEff.defense as any);
