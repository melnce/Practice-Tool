...
export function handleDamageEvent(event: TriggerEventName, activePlayer: Player, context: TriggerContext) {
    ...
    processCandidateTriggers(zones, {
        ...
        predicate: (trigger, cand) => {
            ...
            if (cond.still_alive && (parseInt(cand.card.defense as string, 10) || 0) <= 0) return false;
...
export function handleBuffEvent(event: TriggerEventName, activePlayer: Player, context: TriggerContext) {
    const crests = getCrestCandidates(activePlayer);
    processCandidateTriggers(crests, { event, activePlayer, context });
