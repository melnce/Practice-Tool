...
- **`loot_fused`**: Deduplicated **once per turn per initiator**. This ensures that fusing multiple Loot cards only fires the "On Loot Fuse" effect *once* for that card.
...
4. **Constraints**:
   ...
   - **NEVER** modify `tracking.ts` unless adding a new *global* tracking mode (rare).
