...
export function pickEnhanceTier(card: CardInstance, availablePP: number): { cost: number; effects: Effect[] } | null {
    ...
    for (const t of (card.enhanceTiers || [])) {
        if (availablePP >= t.cost) return { cost: t.cost, effects: t.effects || [] }; // highest affordable
    }
    return null;
}
