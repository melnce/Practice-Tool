...
function handleAddShadows(eff, owner) {
  ...
  if (owner === "blue") state.blueShadows += amt;
  else state.redShadows += amt;
}
...
export function runEffects(effects, owner, sourceCard, context = {}) {
  while (effects.length) {
    ...
    switch (eff.op) {
      ...
      case "congregant_fill_board": congregantFillBoard(owner, sourceCard); break;
      ...
      case "fuse_start": if (handleStartFuse(eff, owner, sourceCard, effects) === "pending") return; break;
      ...
      case "select_hand_summon_artifact_copy": logEvent("summon", { owner, op: eff.op, status: "pending_selection" }); if (handleSelectHandSummonArtifactCopy(eff, owner, effects, sourceCard) === "pending") return; break;
      ...
      case "spellboost_hand": spellboostHand(owner, eff); break;
      ...
      case "summon_named_enemy": { const foe = (owner === "blue") ? "red" : "blue"; summonNamed({ op: "summon_named", name: eff.name, count: eff.count || 1 }, foe); logEvent("summon", { owner: foe, name: eff.name }); break; }
      ...
      case "william_damage_all": { const x = (sourceCard?.currentWilliamDamage ?? sourceCard?.spellboostCount ?? 0) | 0; handleDamageAll({ op: "damage_all", target: "enemy:follower", amount: x }, owner); break; }
