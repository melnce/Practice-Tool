...
import { handleBuff, handleBuffHandTribe, handleBuffLastAddedToHand, handleBuffHandClass, handleSetAttackTo, handleComboRepeatBuff } from "@logic/effects/ops/buff.js";
...
export function runEffects(effects: Effect[], owner: Player, sourceCard: CardInstance | null, context: any = {}) {
    while (effects.length) {
        ...
        switch (eff.op) {
            ...
            case "board_name_gate": handleBoardNameGate(owner, eff, effects); break;
            case "both_max_pp_gate": handleBothMaxPPGate(eff, effects); break;
            ...
            case "damage": if (handleDamage(eff as any, owner, sourceCard, effects, context) === "pending") return; break;
            ...
            case "evolved_self_gate": handleEvolvedSelfGate(eff, owner, sourceCard!, effects); break;
            ...
            case "increase_opponent_hand_cost_eot": { const amt = parseInt(eff.amount as string ?? 1) || 1; applyTempOpponentHandCostMod(owner, amt); break; }
            case "juno_damage": if (handleJunoDamage(eff, owner, sourceCard, effects) === "pending") return; break;
            case "keyword": { const merged = { ...(context || {}), sourceCard }; if (handleKeyword(eff as any, owner, effects, merged) === "pending") return; break; }
            ...
            case "rally_gate": handleRallyGate(owner, eff, effects); break;
            ...
            case "reduce_deck_followers_cost": { const amt = parseInt(eff.amount as string ?? 1) || 1; reduceDeckFollowersCost(owner, amt); break; }
            ...
            case "super_evolved_allied_gate": handleSuperEvolvedAlliedGate(owner, eff, effects); break;
