...
export function getAdapter(ctx: EffectCtx): Adapter {
    if (!ctx.adapter || typeof ctx.adapter !== 'object' || !('render' in (ctx.adapter as object))) {
        throw new Error("[EffectCtx] Invalid Adapter: Expected Adapter object with render method.");
    }
