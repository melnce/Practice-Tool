...
function traceEventEqual(a: EffectTraceEvent | undefined, b: EffectTraceEvent | undefined): boolean {
...
export function formatReplayDiff(diff: ReplayDiff): string {
    ...
    const kinds = Object.keys(diff.summary.leftByKind).sort() as EffectTraceEvent["kind"][];
    ...
    if (diff.summary.leftByOp && diff.summary.rightByOp) {
        ...
        for (const op of top10) {
            ...
            lines.push(`${op.padEnd(30)}${String(l).padEnd(10)}${String(r).padEnd(10)}`);
        }
    }
