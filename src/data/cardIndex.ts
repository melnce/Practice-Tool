...
export function processCard(raw: RawCardData): CardTemplate | null {
    ...
    if (card.type === "Follower" || card.type === "Amulet") {
        ...
        if (card.hasLastWords && keywords.length > 0) {
            const lastWordsKeyword = keywords.find((k): k is { name: string; effects?: unknown[] } =>
                typeof k === 'object' && k !== null && (k as { name?: string }).name === "LastWords"
            );
            card.lastWordsEffects = lastWordsKeyword?.effects ?? [];
        }

        if (card.hasCountdown && keywords.length > 0) {
            const countdownKeyword = keywords.find((k): k is { name: string; turns?: string | number } =>
                typeof k === 'object' && k !== null && (k as { name?: string }).name === "Countdown"
            );
            if (countdownKeyword) {
...
export function getCardDetails(nameOrId: string): CardTemplate | null {
    ...
    return globalCardIndex.byName.get(nameOrId)
...
export function injectCardForTest(card: CardTemplate): void {
    ...
    if (card.id) {
        (globalCardIndex.byId as Map<string, CardTemplate>).set(String(card.id), card);
    }
}
