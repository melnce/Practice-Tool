...
function extractEffectsFromArray(
  effects: Effect[],
  zone: string,
  card: Card,
  results: Map<string, SignatureEntry>,
) {
  ...
  for (const eff of effects) {
    ...
    if (eff.options) {
      for (const opt of eff.options) {
        if (opt.effects)
          extractEffectsFromArray(opt.effects, zone, card, results);
      }
    }
  }
}

function extractKeywordEffects(
  keywords: any[],
  zone: string,
  card: Card,
  results: Map<string, SignatureEntry>,
  ...
  for (const kw of keywords) {
    if (typeof kw === "object" && kw !== null) {
      ...
      if (kw.effects)
        extractEffectsFromArray(
          kw.effects,
          `keyword:${kw.name || "unknown"}`,
          card,
          results,
        );
    }
...
function extractTriggerEffects(
  triggers: any[],
  card: Card,
  results: Map<string, SignatureEntry>,
) {
...
export function processCard(card: Card, results: Map<string, SignatureEntry>) {
  // Fanfare
  if (card.fanfare)
    extractEffectsFromArray(card.fanfare, "fanfare", card, results);

  // Evolve
  if (card.evolve)
    extractEffectsFromArray(card.evolve, "evolve", card, results);

  // Super-evolve
  if (card.superevolve)
    extractEffectsFromArray(card.superevolve, "superevolve", card, results);
  ...
  if (card.keywords)
    extractKeywordEffects(card.keywords, "keyword", card, results);
