...
export function validateGameState(state: GameState): GameStateValidationResult {
      ...
      for (const c of arr) {
        if (c && c.instanceId !== undefined) {
          if (seenIds.has(c.instanceId)) {
            issues.push(
              `Duplicate instanceId ${c.instanceId} found in ${field}`,
            );
          }
  ...
  if (typeof state.blueHP !== "number" || !Number.isFinite(state.blueHP))
    issues.push(`Invalid blueHP: ${state.blueHP}`);
  if (typeof state.redHP !== "number" || !Number.isFinite(state.redHP))
    issues.push(`Invalid redHP: ${state.redHP}`);
