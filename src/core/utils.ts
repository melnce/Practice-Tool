...
function showImageOverlayWithFallback(urls: string[]) {
    ...
    const img = document.getElementById("burnPreviewImg") as HTMLImageElement | null;
    if (!box || !img) return;
...
function burnPreview(card: CardInstance | any) {
    ...
    const img = document.getElementById("burnPreviewImg") as HTMLImageElement | null;
    if (!box || !img) return;
    img.onload = null; img.onerror = null;          // no fallback for burns
    (img as any).src = card?.base_image || card?.image || "";
    box.style.display = "block";
    setTimeout(() => (box.style.display = "none"), 900);
}

export function pushToHand(hand: CardInstance[], card: CardInstance | any): boolean {
 ...
 */
export function drawCard(hand: CardInstance[], deck: CardInstance[], owner: "blue" | "red" | null = null): boolean {
    if (!owner) {
        if (hand === state.blueHand) owner = "blue";
        else if (hand === state.redHand) owner = "red";
    }
    ...
    if (!deck || deck.length === 0) {
        if (typeof document !== "undefined") {
            ...
            if (iWinOnDeckout) {
                if (opp === "blue") state.blueHP = 0; else state.redHP = 0;
            } else {
                if (owner === "blue") state.blueHP = 0; else state.redHP = 0;
            }
