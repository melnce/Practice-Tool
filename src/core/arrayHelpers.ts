...
export function mustGet<T>(arr: T[], index: number, context?: string): T {
  if (index < 0 || index >= arr.length) {
    throw new Error(
      `mustGet: index ${index} out of bounds [0, ${arr.length}) ${context ? `in ${context}` : ""}`,
    );
  }
  const val = arr[index];
  if (val === undefined) {
    throw new Error(
      `mustGet: element at index ${index} is undefined ${context ? `in ${context}` : ""}`,
    );
  }
  return val;
}
...
export function assertPresent<T>(
  ...
  if (val === null || val === undefined) {
    throw new Error(
      `assertPresent: value is ${val} ${context ? `in ${context}` : ""}`,
    );
  }
  return val;
}
