...
results.slice(0, 30).forEach((r, i) => {
    ...
    console.log(`${String(i + 1).padStart(2)}. ${nameStr} ${selfMs.padStart(8)}ms (${pctStr}%)  ${locStr}`);
});
...
fileResults.forEach(([file, time], i) => {
    ...
    console.log(`${String(i + 1).padStart(2)}. ${file.substring(0, 55).padEnd(55)} ${ms.padStart(8)}ms (${pct.padStart(5)}%)`);
});
...
const engineResults = results.filter(r =>
