...
function extractCrestsFromCard(card: CardDef): CrestDef[] {
        ...
        for (const effect of effects) {
            ...
            if (e.op === "crest" && (e.action === "gain" || !e.action)) {
                ...
                crests.push({
                    ...
                    triggers: Array.isArray(e.triggers) ? e.triggers as CrestTrigger[] : undefined,
                    effects: Array.isArray(e.effects) ? e.effects : undefined,
                    on_gain: Array.isArray(e.on_gain) ? e.on_gain : undefined,
                    keywords: Array.isArray(e.keywords) ? e.keywords as string[] : undefined,
            ...
            if (Array.isArray(e.else_effects)) processEffects(e.else_effects as unknown[]);
    ...
    const evolveEff = card.evolve && !Array.isArray(card.evolve)
        ? (card.evolve as { effects?: unknown[] }).effects
        : card.evolve;
    processEffects(evolveEff as unknown[]);

    const superEff = card.superevolve && !Array.isArray(card.superevolve)
    ...
    if (Array.isArray(card.keywords)) {
        for (const kw of card.keywords) {
            if (kw && typeof kw === "object" && Array.isArray((kw as Record<string, unknown>).effects)) {
...
function formatTrigger(trigger: CrestTrigger): string {
    ...
    if (Array.isArray(trigger.effects) && trigger.effects.length > 0) {
        const effectsStr = trigger.effects.map(e => {
...
function formatEffect(e: Record<string, unknown>, depth = 0): string {
    ...
    switch (op) {
        ...
        case "draw": {
            ...
            if (filters?.defense_eq) filterParts.push(`defense=${filters.defense_eq}`);
        ...
        case "keyword": {
            ...
            if (action === "grant_trigger") {
                ...
                if (triggers.length > 0) {
                    const triggerDescs = triggers.map((t: any) => {
                        const event = t.type || t.event || "trigger";
                        const effects = Array.isArray(t.effects) ? t.effects : [];
                        const effectsStr = effects.map((eff: any) => formatEffect(eff, depth + 1)).join("; ");
            ...
            const keywords = Array.isArray(e.keywords)
                ? e.keywords.map(k => typeof k === "string" ? k : (k as { name?: string })?.name || "?").join(", ")
                : e.name || "keyword";
        ...
        case "earth_rite": {
            ...
            if (Array.isArray(e.effects) && e.effects.length > 0) {
                const nested = e.effects.map(eff => formatEffect(eff as Record<string, unknown>, depth + 1)).join("; ");
                result += ` → ${nested}`;
            }
            return result;
        }
        ...
        case "gate": {
            ...
            if (Array.isArray(e.else_effects) && e.else_effects.length > 0) {
                const elseNested = e.else_effects.map(eff => formatEffect(eff as Record<string, unknown>, depth + 1)).join("; ");
        ...
        case "repeat_effect": {
            const times = e.count_source ? `X (${e.count_source})` : `${e.count || 1}`;
...
function formatCrestBehavior(crest: CrestDef): string {
    ...
    if (crest.on_gain && crest.on_gain.length > 0) {
        const effectsStr = crest.on_gain.map(e => {
            if (typeof e === "object" && e !== null) {
                return formatEffect(e as Record<string, unknown>);
            }
            return String(e);
    ...
    if (crest.triggers && crest.triggers.length > 0) {
        const triggersStr = crest.triggers.map(t => formatTrigger(t)).join("\n  ");
        parts.push(`Triggers:\n  ${triggersStr}`);
    }

    // Effects (Last Words / Countdown completion)
    if (crest.effects && crest.effects.length > 0) {
        const effectsStr = crest.effects.map(e => {
            if (typeof e === "object" && e !== null) {
                return formatEffect(e as Record<string, unknown>);
            }
            return String(e);
...
async function main() {
    ...
    console.log(`🔍 Validating ${crestsToValidate.length} crests with ${modelName} (batch size ${BATCH_SIZE})...\n`);
    ...
    for (let i = 0; i < crestsToValidate.length; i += BATCH_SIZE) {
        ...
        for (let j = 0; j < batchResults.length; j++) {
            ...
            if (verbose || result.verdict === "FAIL") {
                console.log(`${icon} [${crestIndex + 1}/${crestsToValidate.length}] ${batch[j].name}: ${result.verdict}`);
                if (result.verdict === "FAIL") {
                    console.log(`   └─ ${result.reasoning}`);
                }
            } else {
                process.stdout.write(`\r[${crestIndex + 1}/${crestsToValidate.length}] ✅${passed} ❌${failed} ❓${unclear} ⚠️${errors}`);
            }
        }
    ...
    console.log(`✅ Pass:  ${passed} (${((passed / crestsToValidate.length) * 100).toFixed(1)}%)`);
    console.log(`❌ Fail:  ${failed} (${((failed / crestsToValidate.length) * 100).toFixed(1)}%)`);
    ...
    if (failedCrests.length > 0) {
        console.log(`📄 Failed crests: ${path.join(outputDir, "crest_failed.json")}`);
    }
}
