...
interface BenchEnvModule {
    ...
    benchTiming: {
        ...
        summary: () => {
            ...
            byStage: Record<string, { count: number; ms: number; pct: number; avgUs: number }>;
            totalApplyMs: number;
        };
    };
}
...
function selectAction(actions: BenchAction[], stepCount: number, policy: Policy): BenchAction {
...
function stepLoop(
    ...
    while (stepCount < steps) {
        ...
        if (actions.length === 0) {
            console.warn(`[WARNING] No legal actions at step ${stepCount}. Resetting.`);
            const resetStart = performance.now();
            env.reset(resetSeed++);
            resetCheckMs += performance.now() - resetStart;
            resets.error++;
    ...
    // Avg game length
    const avgGameLength = terminalGameLengths.length > 0
        ? terminalGameLengths.reduce((a, b) => a + b, 0) / terminalGameLengths.length
...
function microGetLegalActions(env: BenchEnvModule, seed: number, iterations: number): MicroResult {
    env.reset(seed);
    let totalActions = 0;
 ...
 */
function microApplyAction(env: BenchEnvModule, seed: number, iterations: number): MicroResult {
...
function printResult(result: SimResult, label: string): void {
    ...
    const endTurnPerSec = formatRate(result.actionCounts.END_TURN || 0, result.durationMs);
    const avgActionsPerStep = (result.totalLegalActions / result.steps).toFixed(1);
    const duration = (result.durationMs / 1000).toFixed(2);

    console.log(`\n[${label}]`);
    console.log(`  Steps: ${formatNumber(result.steps)} in ${duration}s`);
    console.log(`  Throughput: ${stepsPerSec} steps/sec`);
    console.log(`  Turns/sec: ${turnsPerSec} (from turn delta: ${result.turnDelta})`);
    console.log(`  END_TURN applied/sec: ${endTurnPerSec}`);
    console.log(`  Avg legal actions/state: ${avgActionsPerStep}`);
    console.log(`  Avg game length: ${result.stepsPerTerminal.toFixed(1)} steps/terminal`);
    console.log(`  Resets: manual=${result.resets.manual}, terminal=${result.resets.terminal}, error=${result.resets.error}`);
}
...
function printTimeBreakdown(timing: TimeBreakdown): void {
    ...
    console.log(`\n[Time Breakdown]`);
    console.log(`  getLegalActions: ${timing.legalActionsMs.toFixed(1)}ms (${legalPct}%)`);
    console.log(`  applyAction:     ${timing.applyActionMs.toFixed(1)}ms (${applyPct}%)`);
    console.log(`  reset/terminal:  ${timing.resetCheckMs.toFixed(1)}ms (${resetPct}%)`);
    console.log(`  total:           ${timing.totalMs.toFixed(1)}ms`);
}

function printActionDistribution(counts: Record<string, number>, total: number): void {
    console.log("\n[Action Distribution]");
...
function printApplyBreakdown(env: BenchEnvModule): void {
    ...
    console.log("  Stage              | Count      | Total ms   | % of stages | Avg µs/call");
    console.log("  -------------------|------------|------------|-------------|------------");
    for (const [stage, data] of Object.entries(summary.byStage)) {
        ...
        const avgStr = data.avgUs.toFixed(2).padStart(10);
        console.log(`  ${stage.padEnd(18)} | ${countStr} | ${msStr} | ${pctStr}% | ${avgStr}`);
    }
}
...
async function main(): Promise<void> {
    ...
    console.log("╔══════════════════════════════════════════════════════════════╗");
    console.log("║           SHADOWVERSE ENGINE BENCHMARK                        ║");
    console.log("╚══════════════════════════════════════════════════════════════╝");
    console.log(`Version: ${versionInfo.version} | Commit: ${versionInfo.commit}`);
    console.log(`Date: ${new Date().toISOString()}`);
    console.log(`Seed: ${config.seed}`);
    console.log(`Policy: ${config.policy}`);
    console.log(`Warmup: ${formatNumber(config.warmupSteps)} | Main: ${formatNumber(config.mainSteps)}`);
    console.log(`Breakdown: ${config.breakdown ? "ENABLED" : "disabled"}`);
    console.log(`History: ${process.env.DISABLE_HISTORY === "1" ? "DISABLED (optimized)" : "ENABLED (slow)"}`);
    console.log(`UID Enrichment: ${process.env.DISABLE_UID_ENRICH === "1" ? "DISABLED (optimized)" : "ENABLED"}`);
    console.log("");
    ...
    try {
        const historyModule = await import(resolve(__dirname, "..", loadedFrom === "DIST" ? "dist/core/history.js" : "src/core/history.ts"));
        const histEnabled = historyModule.isHistoryEnabled?.() ?? "unknown";
        console.log(`History runtime check: isHistoryEnabled()=${histEnabled}`);
        if (histEnabled === true) {
            console.warn("⚠ WARNING: History snapshots are ENABLED - benchmark will be slow!");
        }
    } catch (e) {
    ...
    console.log(`Warmup: ${formatNumber(warmupSteps)} steps in ${(warmupMs / 1000).toFixed(2)}s (${warmupRate} steps/sec)`);
    console.log(`Warmup resets: terminal=${warmupResets.terminal}, error=${warmupResets.error}`);
    ...
    if (config.profile) {
        ...
        console.log(`Running for ${config.profileSeconds} seconds with CPU profiling...`);
        ...
        const profilePath = join(profilesDir, `cpu-profile-${config.policy}-${timestamp}.cpuprofile`);
        ...
        console.log(`  Steps: ${formatNumber(profileSteps)} in ${(profileMs / 1000).toFixed(2)}s`);
        console.log(`  Throughput: ${profileRate} steps/sec`);
        console.log(`  Resets: terminal=${profileResets.terminal}, error=${profileResets.error}`);
    ...
    // Run the main loop
    const result = stepLoop(env, config.mainSteps, config.policy, measureSeed + 1);
    ...
    const microLegal = microGetLegalActions(env, config.seed + 5000, config.microIters);
    printMicro(microLegal);

    const microApply = microApplyAction(env, config.seed + 6000, config.microIters);
    ...
    console.log(`  First Board: ${stats.firstBoardSize}, Second Board: ${stats.secondBoardSize}`);
    console.log(`  First Hand: ${stats.firstHandSize}, Second Hand: ${stats.secondHandSize}`);
