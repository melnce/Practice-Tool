...
const KNOWN_OPS = new Set([
  ...
  "transform",
  "transform_random",
  "transform_random_enemy",
  "swap_attack_defense",
  "swap_attack_defense_all",
  // Buffs
  "buff",
  "buff_self",
  "dynamic_buff_self",
...
function main() {
  const { signatures: results, setStats } = scanSets(
    SETS_DIR,
    SETS_TO_SCAN,
    TOKENS_FILE,
  );
  ...
  const sortedEntries = [...results.values()].sort((a, b) =>
    a.normalizedSig.op.localeCompare(b.normalizedSig.op),
  );
  ...
  for (const [op, entries] of [...byOp.entries()].sort((a, b) =>
    ...
    for (const entry of entries) {
      ...
      lines.push(
        `| \`${sig}\` | ${entry.zone} | ${entry.representativeCard} | ${entry.count} |`,
      );
    }
    lines.push("");
  }
