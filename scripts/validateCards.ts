...
function formatTarget(target: unknown, select?: number): string {
  ...
  const isPlural =
    (!select && select !== 0) || (select !== undefined && select > 1);
    ...
    for (const effect of arr) {
      ...
      switch (e.op) {
        ...
        case "damage": {
          ...
          if (dist) {
            if (dist.includes("random")) {
              targetStr = targetStr
                .replace(/^all /, "random ")
                .replace(/^enemy/, "random enemy");
              if (!targetStr.includes("random"))
                targetStr = `random ${targetStr}`;
          ...
          if (times > 1) {
            if (dist === "random") {
              ...
              effects.push(
                `${trigger}: Deal ${amt}${dynamicAmt}${addAmt} damage to ${times} ${baseTarget}s`,
              );
            } else {
              // Repeated hits: "Deal X damage to random enemy follower 2 times"
              effects.push(
                `${trigger}: Deal ${amt}${dynamicAmt}${addAmt} damage to ${targetStr} ${times} times`,
              );
            }
          } else {
            ...
            if (dist === "random" && times === 1) {
              finalTarget = targetStr
                .replace(/followers$/, "follower")
                .replace(/enemies$/, "enemy");
            }
            effects.push(
              `${trigger}: Deal ${amt}${dynamicAmt}${addAmt} damage to ${finalTarget}`,
            );
          }
          break;
        }
        case "restore": {
          ...
          const dynamicAmt = e.amount_source ? ` (X = ${e.amount_source})` : "";
          effects.push(
            `${trigger}: Restore ${amt}${dynamicAmt} to ${formatTarget(e.target || "leader", e.select as number)}`,
          );
          break;
        }
        case "summon": {
          if (e.source === "graveyard") {
            ...
            const sel =
              e.source === "self" ? "this" : e.select ? `selected` : "random";
            const filterF = e.filter as any;
            const filterParts = [];
            if (filterF?.type) filterParts.push(`type=${filterF.type}`);
            if (filterF?.cost_lte)
              filterParts.push(`cost<=${filterF.cost_lte}`);
        ...
        case "stat": {
          ...
          const def =
            e.defense !== undefined ? `/${formatStat(e.defense, isSet)}` : "";
          ...
          const filter = e.filter as
            ...
            | undefined;
          const filterStr =
            filter?.class || filter?.subtype || filter?.trait || "";
          let target = filterStr
            ? `${filterStr} ${formatTarget(e.target, e.select as number)}`
            : formatTarget(e.target, e.select as number);
          ...
          const filterPosition =
            typeof e.filter === "string" ? e.filter : undefined;
          if (filterPosition === "leftmost") {
            ...
            target = target
              .replace(/^all /, "random ")
              .replace(/^enemy/, "random enemy")
              .replace(/^ally/, "random allied")
              .replace(/^allied/, "random allied");
            if (!target.includes("random")) target = `random ${target}`;
          }

          const condition = e.condition as
            | { not_self?: boolean; tribe?: string; class?: string }
            | undefined;
          if (condition?.tribe) {
          ...
          const keywords =
            (e.keywords as (string | { name: string })[])
              ?.map((k) => (typeof k === "string" ? k : k.name))
              .join(", ") || "";
          const effectStr = [stats, keywords].filter(Boolean).join(" and ");

          let durationStr = "";
          if (e.duration === "opponent_turn_end")
            durationStr = " until the end of your opponent's turn";
          else if (e.until_eot) durationStr = " until the end of the turn";

          effects.push(
            `${trigger}: ${e.action || "give"} ${effectStr} to ${target}${durationStr}`,
          );
          break;
        }
        case "keyword": {
          const kws =
            (e.keywords as unknown[])
              ?.map((k) =>
                typeof k === "string"
                  ? k
                  : (k as { name?: string })?.name || "unknown",
              )
              .join(", ") ||
            e.name ||
            "unknown";
          const filter = (e.filter || e.filters) as
            | { class?: string; subtype?: string; trait?: string }
            | undefined;
          const filterStr =
            filter?.class || filter?.subtype || filter?.trait || "";
          let target = filterStr
          ...
          effects.push(
            `${trigger}: ${e.action || "grant"} ${kws} to ${target}`,
          );
          break;
        }
        case "destroy": {
          const count = (e.count as number) || (e.select as number);
          const condObj = e.condition as
            | {
                not_self?: boolean;
                tribe?: string;
                class?: string;
          ...
          if (e.exclude && Array.isArray(e.exclude)) {
            const excludes = e.exclude
              .map((ex: string) => ex.replace("context.", ""))
              .join(", ");
          ...
          if (dist === "random") {
            ...
            target = target
              .replace("any allied cards", "allied cards")
              .replace("all ally:any", "all allied cards");
          }

          const dynamicCount = e.count_source
            ? `X (X = ${e.count_source})`
            : count && count > 1
              ? count
              : "";
        ...
        case "evolve":
          ...
          effects.push(
            `${trigger}: ${mode} ${formatTarget(e.target || "self", e.select as number)}`,
          );
          break;
        case "counter":
          effects.push(
            `${trigger}: ${e.action} ${e.amount || 1} to ${e.key} counter`,
          );
          break;
        case "search": {
          ...
          effects.push(
            `${trigger}: Search for ${e.count || 1} card(s)${filterStr}`,
          );
          break;
        }
        case "cost": {
          effects.push(
            `${trigger}: Cost: ${e.action || e.mode} ${e.amount} to ${formatTarget(e.target || "cards", e.select as number)}`,
          );
          break;
        }
        case "deck": {
        ...
        case "return":
          effects.push(
            `${trigger}: Return ${formatTarget(e.target, e.select as number)} to ${e.destination}`,
          );
          break;
        case "earth_rite":
        case "necromancy": {
        ...
        case "banish": {
          ...
          let banishTarget = (e.scope as string)
            ? (e.scope as string)
            : formatTarget(e.target, banishCount);
        ...
        case "gate": {
          ...
          if (
            ...
            if (
              Array.isArray(e.else_effects) &&
              (e.else_effects as unknown[]).length > 0
            ) {
              extractFromEffects(
                e.else_effects as unknown[],
                `${trigger} (base)`,
              );
            }
                  ...
                  : condition === "enhance"
                    ...
                    : condition === "skybound_art"
                      ...
                      : condition === "overflow"
                        ...
                        : `${condition} ${count}`;
              extractFromEffects(
                e.effects as unknown[],
                `${trigger} IF ${condLabel}`,
              );
            }
          } else {
            // Generic conditional
        ...
        case "mode": {
          ...
          if (options && options.length > 0) {
            const countStr =
              count > 1 ? `Choose ${count} modes` : "Choose a mode";
            effects.push(
              `${trigger}: ${countStr} (${options.length} options):`,
            );
        ...
        case "pp":
          if (e.action === "recover") {
            ...
            effects.push(
              `${trigger}: ${e.action || "modify"} ${e.amount || 1} PP`,
            );
          }
          break;
        case "spellboost":
          if (e.target === "self" && e.mode === "set") {
            effects.push(
              `${trigger}: Set spellboost count to ${e.amount ?? 0}`,
            );
          } else {
        ...
        case "crest": {
          ...
          if (e.action === "pay_counter") {
            effects.push(
              `${trigger}: Pay ${e.amount} ${e.counter} from ${crestName}`,
            );
            if (Array.isArray(e.on_success_effects)) {
              extractFromEffects(
                e.on_success_effects as unknown[],
                `  └─ On Success`,
              );
            }
            ...
            effects.push(
              `${trigger}: Advance Crest: ${crestName} by ${e.amount || 1}`,
            );
            ...
            if (e.description) {
              ...
              for (const tr of e.triggers as any[]) {
                const eventName = String(tr.event || "trigger").replace(
                  /_/g,
                  " ",
                );
                let trLabel = `  └─ Ability: On ${eventName}`;
                if (tr.condition) {
                  const condStr = Object.entries(tr.condition)
                    .map(([k, v]) => `${k}=${v}`)
                    .join(", ");
                  trLabel += ` (IF ${condStr})`;
                }
        ...
        case "countdown": {
          ...
          const dynamicAmt = e.amount_source ? ` (X = ${e.amount_source})` : "";
          effects.push(
            `${trigger}: Countdown ${action} by ${amt}${dynamicAmt}`,
          );
          break;
        }
        case "mode_bonus":
          effects.push(
            `${trigger}: Increase mode selection count by ${e.amount || 1}`,
          );
          break;
        case "reanimate":
    ...
    for (const kw of card.keywords) {
      if (typeof kw === "string") {
        ...
        if (k.name === "LastWords" && Array.isArray(k.effects)) {
          ...
          String(k.name).toLowerCase() === "enhance" &&
          Array.isArray(k.effects)
        ) {
  ...
  if (card.description) {
    for (const kw of PASSIVE_KEYWORDS) {
      ...
      if (
        ...
        !effects.some((e) => e.toLowerCase().includes(kw.toLowerCase()))
      ) {
...
async function validateCard(card: CardDef): Promise<ValidationResult> {
  ...
  if (stateChanges.length === 0) {
    ...
    stateChangesText =
      stateChanges.slice(0, MAX_EFFECTS_LINES).join("\n") +
...
async function main() {
  ...
  console.log(
    `🔍 Validating ${cards.length} cards with ${modelName} (batch size ${BATCH_SIZE})...\n`,
  );
  ...
  for (let i = 0; i < cards.length; i += BATCH_SIZE) {
    ...
    for (let j = 0; j < batchResults.length; j++) {
      ...
      if (verbose || result.verdict === "FAIL") {
        console.log(
          `${icon} [${cardIndex + 1}/${cards.length}] ${batch[j].name}: ${result.verdict}`,
        );
        ...
        process.stdout.write(
          `\r[${cardIndex + 1}/${cards.length}] ✅${passed} ❌${failed} ❓${unclear} ⚠️${errors}`,
        );
      }
    }
  ...
  console.log(
    `✅ Pass:  ${passed} (${((passed / cards.length) * 100).toFixed(1)}%)`,
  );
  console.log(
    `❌ Fail:  ${failed} (${((failed / cards.length) * 100).toFixed(1)}%)`,
  );
