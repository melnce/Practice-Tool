 ...
 * (except this generated backlog file) contains the card id AND at least one
 * keyword from the answer. A keyword is a 4+ letter token that is not in
 * QA_COVERAGE_STOPWORDS.
 */
...
export function loadTestCorpus(
  ...
  skipNames: string[] = ["official-qa.test.ts"],
...
export function renderOfficialReportMarkdown(
  ...
  const lines = [
    ...
    "## Q&A coverage",
    "",
    `Pin rule: a Q&A is **pinned** if any file under \`tests/\` other than \`tests/unit/official-qa.test.ts\` contains the card id **and** at least one 4+ letter keyword from the answer after dropping ${QA_COVERAGE_STOPWORDS.size} stopwords. Otherwise **unpinned**.`,
    "",
