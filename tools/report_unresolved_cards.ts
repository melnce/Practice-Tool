...
export function generateReportData(cards: any[]): {
  stats: any;
  ranking: UnresolvedEntry[];
} {
  ...
  const ranking = Array.from(clauseMap.values()).sort(
    (a, b) => b.count - a.count,
  );
  ...
  const stats = {
    ...
    cardsWithUnresolved,
    percentUnresolved: cards.length
      ? ((cardsWithUnresolved / cards.length) * 100).toFixed(1) + "%"
      : "0.0%",
    totalUnresolvedClauses,
  };
...
async function main() {
  ...
  console.log(
    `Cards with Issues: ${stats.cardsWithUnresolved} (${stats.percentUnresolved})`,
  );
...
if (
  ...
  process.argv[1].endsWith("report_unresolved_cards.ts")
) {
