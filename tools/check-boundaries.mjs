...
try {
  fs.unlinkSync("boundary_errors.txt");
} catch (e) {}
...
for (const sourceFile of sourceFiles) {
  ...
  const checkSpecifier = (specifier, nodeType, lineNum) => {
        ...
        const declaration =
          nodeType === "import"
            ? sourceFile.getImportDeclaration(
                (d) => d.getModuleSpecifierValue() === specifier,
              )
            : sourceFile.getExportDeclaration(
                (d) => d.getModuleSpecifierValue() === specifier,
              );
    ...
    if (
      resolvedFile &&
      resolvedFile.toLowerCase().replace(/\\/g, "/").includes(FORBIDDEN_LAYER)
    ) {
  ...
  const callExpressions = sourceFile.getDescendants().filter((n) => {
    ...
    return (
      n.getKindName() === "CallExpression" &&
      n.getExpression().getKindName() === "ImportKeyword"
    );
  });
