...
**Shipped deck ids** live in **`decks/manifest.json`** (generated by `npm run decks:discover`). That manifest is the single source of truth for which deck files are shipped: soak pairings, deck validation tests, and `check:deck-rotation` all read it. Per-deck fixture references elsewhere (L2 tests, UI defaults, soak repro JSON) are intentional and only need edits when _that specific deck_ is removed.

### Adding or removing a shipped deck

```bash
# Add:    create decks/<id>.json, then:
# Remove: delete decks/<id>.json, then:
npm run decks:discover
npm run check
```

No `.gitignore` un-ignore lines, no soak array, and no deck-library test list to edit. `check:deck-rotation` fails if the soak pool or manifest drifts from `decks/` on disk.

Scratch imports saved locally should use a `*.local.json` suffix under `decks/` (gitignored). Session paste/file imports stay in memory via `importedDeckStore`, not on disk.

Soak fixture JSON under `tests/fixtures/soak/` embed deck ids as repro artifacts — do not edit those when rotating shipped decks unless you are re-recording soak traces.
