...
**Supersedes** the gears-only part of "Artifact fuse chain — 2026-09-05". The Gears' printed
"Fuse: Artifact amulets" was right; the recollection was wrong. Gear of Ambition / Gear of Remembrance
(`90071210` / `90071220`): partners = any Artifact **amulet** (the gear fused into still decides the body).
Striker / Fortifier Artifact (`90072110` / `90072120`): partners = any Artifact **card**. Measured on the
rotation pool: the only Artifact amulets are the two Gears, so behaviour is unchanged today; the rule now
follows the text. Engine: `src/logic/effects/ops/fuse/fuse.artifact.ts` — **not changed in this PR**; if it
hard-codes gears-only, report the lines in the PR body as a follow-up, do not fix here.
