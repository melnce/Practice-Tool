Mirror of `melnce/arena` `docs/trace-format.md` @ bb769d3; the arena copy is canonical
...
```json
{
  "i": 0,
  "action": {
    "mulligan": { "player": "a", "swap": [true, false, false, true] }
  },
...
```jsonc
{
  ...
  "faith": 0,
  "field": [
    /* 0..4, omitted empties are null */
  ],
  "hand": [
    { "card": "10131320", "cost": 1, "vars": { "X": 2 }, "skybound": 0 },
  ],
  "leader_defense": 20,
  "leader_max": 20,
  "pp": 1,
...
- A card chosen from hand, deck or cemetery is `choose {card: "<id>"}`; copies of one id collapse to one option among the offered options; the engine picks any copy.
- When a `choose {card}` names an id with several copies among the offered options, the copy at the lowest position among the offered options is taken.
...
## Practice-Tool mapping

| Trace key                 | Engine field / expression                                     | Notes                                                                                                                                                                                                                                                                 |
| ------------------------- | ------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `active`                  | `state.activePlayer` → `"a"` if `"first"`, else `"b"`         |                                                                                                                                                                                                                                                                       |
| `phase`                   | `state.phase` + pending prompts                               | `"mulligan"` when `phase === "mulligan"`; `"choice"` when `pendingTargetEffect` or `pendingModeChoice`; `"main"` when `phase` is `"main"` or `"playing"`; `"terminal"` when `phase === "gameover"` or `isGameOver`; `"end"` unused (engine has no distinct end phase) |
| `turn`                    | `0` in mulligan; else `state.roundCount`                      | round counter (both players' first turns are turn 1)                                                                                                                                                                                                                  |
| `winner`                  | `state.winner` mapped to `"a"`/`"b"`                          | `null` when unset                                                                                                                                                                                                                                                     |
| `players.a` / `players.b` | `state.players.first` / `state.players.second`                | deck A loads into `first` (player `"a"`)                                                                                                                                                                                                                              |
| `banished`                | `players[*].banish` card ids                                  | sorted multiset                                                                                                                                                                                                                                                       |
| `cemetery`                | `players[*].graveyard` card ids                               | sorted multiset                                                                                                                                                                                                                                                       |
| `combo`                   | `players[*].playsThisTurn`                                    | combo = cards played this turn                                                                                                                                                                                                                                        |
| `crests[].id`             | crest `name` resolved via card index                          | `Faith: <name>` → `faith:<card id>`; other crests → `crest:<granting card id>` by name lookup                                                                                                                                                                         |
| `crests[].countdown`      | `crest.countdown`                                             | omitted when crest has no countdown                                                                                                                                                                                                                                   |
| `deck`                    | `players[*].deck` card ids                                    | sorted multiset; deck order not serialized                                                                                                                                                                                                                            |
| `earth`                   | sum of `counters.earth` on board amulets                      | `{earth_counter_sum}` template                                                                                                                                                                                                                                        |
| `ep`                      | `players[*].evoCharges`                                       | evolution points remaining                                                                                                                                                                                                                                            |
| `evolves_used`            | `players[*].evoCount`                                         | total evolves this match                                                                                                                                                                                                                                              |
| `faith`                   | sum of `crest.counters.faith` across crests                   | player-level faith gauge                                                                                                                                                                                                                                              |
| `field[].*`               | board slot `players[*].board[i]`                              | five slots; `null` when empty                                                                                                                                                                                                                                         |
...
| `field[].evolved`         | `card.hasEvolved` or `card.evoType` present                   |                                                                                                                                                                                                                                                                       |
| `field[].super`           | `card.evoType === "super"` or `card.superEvolved`             |                                                                                                                                                                                                                                                                       |
| `field[].traits`          | boolean trait flags on instance (`ambush`, `ward`, `rush`, …) | sorted; see Conventions — not `keywords[].name`                                                                                                                                                                                                                       |
| `field[].vars`            | `card.counters` keys X/Y/Z only                               | emitted only when present                                                                                                                                                                                                                                             |
| `field[].granted`         | instance trigger tags minus fresh printed tags                | sorted schema trigger names; see Conventions                                                                                                                                                                                                                          |
| `hand[].card`             | `card.id`                                                     | hand order preserved                                                                                                                                                                                                                                                  |
| `hand[].cost`             | effective cost (`effectiveCost` or `cost`)                    | integer                                                                                                                                                                                                                                                               |
| `hand[].vars`             | hand card `counters` X/Y/Z                                    | only when present                                                                                                                                                                                                                                                     |
| `hand[].skybound`         | `card.skyboundArtEvolvesWitnessed`                            | only on hand copies with Skybound Art; omitted when 0/absent                                                                                                                                                                                                          |
| `leader_defense`          | `players[*].hp`                                               |                                                                                                                                                                                                                                                                       |
| `leader_max`              | `players[*].maxHP`                                            |                                                                                                                                                                                                                                                                       |
| `pp`                      | projected from `players[*].pp`                                | `0` until the player's first turn has started; see Conventions                                                                                                                                                                                                        |
| `pp_bonus`                | projected from `players[*].bonusPpOrb`                        | `0` until the player's first turn has started                                                                                                                                                                                                                         |
| `pp_max`                  | projected from `players[*].maxPP`                             | `0` until the player's first turn has started                                                                                                                                                                                                                         |
| `rally`                   | `players[*].rally`                                            |                                                                                                                                                                                                                                                                       |
| `sep`                     | `players[*].superEvoCharges`                                  | super-evolution points                                                                                                                                                                                                                                                |
| `shadows`                 | `players[*].shadows`                                          |                                                                                                                                                                                                                                                                       |
| `first` (header)          | always `"a"`                                                  | engine does not coin-flip; `first` slot is deck A                                                                                                                                                                                                                     |
| `deck_a` / `deck_b`       | input deck multisets                                          | sorted id array, 40 cards each                                                                                                                                                                                                                                        |
