...
1. **Rally:** Rally counts followers that entered play (played or summoned). Crystallize plays the card _as an amulet_ and Accelerate _as a spell_, so this engine does **not** increment Rally for the alternate-form play itself (summons created by Accelerate text or by a Crystallize amulet's Last Words still increment Rally via the summon path). **Confirm with owner** whether any printed "play a follower" / Rally edge case should treat the alternate play differently.
