...
- **Super-Evolution Points (SEP):** new to WB, each player has 2 (purple pips, separate from EP's yellow). Like EP they are finite and never refreshed, usable from turn 6 (second player) or turn 7 (first). Super-evolution is a distinct action; when both EP and SEP are available and conditions are met, you choose which to spend. A follower can be in an evolved state only once: you cannot evolve an already-evolved or super-evolved follower, and you cannot upgrade an evolved follower into a super-evolved one. _Test case:_ using SEP on a follower already evolved with EP is rejected. An effect that auto-evolves a follower does not consume your EP but does set its evolved state (so it cannot then be evolved manually).
...
**Owner ruling — multi-tier Enhance (2026-09-02):** The parenthetical above ("which picks the highest higher cost you can afford") reads as though only one Enhance tier applies. In this engine `pickEnhanceTiers` returns **every** affordable tier (cost ≤ PP paid), and that is intentional. The only multi-tier Enhance card in the pool is **Noel IV, Ruthless Warlord** (`10624110`): at 8 PP both Enhance (7) and Enhance (8) fire, summoning two Fearless Soldiers (one Drain, one Storm) in addition to the Fanfare Bane soldier. Do not change `pickEnhanceTiers` to keep only the highest tier.
...
2. **Spellboost:** Playing via Accelerate is playing "as a spell." This engine runs the normal spell-play Spellboost pass. **Confirm** if any card text should exclude Accelerate from Spellboost.
3. **Cost modification of N:** Treated like Enhance — alternate cost N is fixed. **Confirm** if any effect is meant to raise/lower Crystallize/Accelerate N specifically.
4. ~~**Original cost under Accelerate / Crystallize:**~~ **Settled** — see **Owner ruling — Accelerate / Crystallize original cost (2026-08-12)** above. Printed base cost is preserved; the 2018 original-Shadowverse tweet does not apply to Worlds Beyond.
